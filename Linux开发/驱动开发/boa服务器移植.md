...
```bash
curl -o config.guess https://raw.githubusercontent.com/autotools-mirror/config/master/config.guess
curl -o config.sub https://raw.githubusercontent.com/autotools-mirror/config/master/config.sub
```
...
### 10.在src目录下创建4个文件夹：
    ...
    $ mkdir www log etc bin

www存放网页目录，log存放日志目录，bin存放make出来的二进制文件

进入www文件夹,创建存放CGI的文件目录
...
以下是配置文件部分修改过的内容，根据需要修改
    ...
    DocumentRoot /root/boa-0.94.13/src/www
    UserDir public_html
    ...
    Alias /doc /usr/doc
    ScriptAlias  /cgi-bin/ /root/boa-0.94.13/src/www/cgi-bin/
