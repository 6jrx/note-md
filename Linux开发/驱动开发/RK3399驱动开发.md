## 一、刷入初始出厂固件

准备工具：`AndroidTool.exe`

![](img\010.png)

进入刷机模式

- normal模式：正常启动

- loader模式：升级内核时
    - 命令：`reboot loader`
    - 按键： 按住`recovery`键之后再按下`reset`键等3-5秒

- maskrom模式：刷入完全版固件，完全初始化。包括所有需要刷入soc的内容


开始操作刷入

1. 进入loader

```bash
reboot loader
```

2. 用软件进入maskrom

![](img\011.png)

3. 执行默认的刷机行为

![](img\012.png)

> 确保固件文件都在同目录下

刷入固件后自动重启

账号密码：`root` `fa`


## 二、交叉编译工具链

编译工具：gcc-linaro-7.3.1-2018.05-x86_64_aarch64-linux-gnu

下载地址：`https://releases.linaro.org/components/toolchain/binaries/`

配置工具链到虚拟机环境中

1. 添加到环境变量

```bash
export PATH=~/gcc-linaro-7.3.1-2018.05-x86_64_aarch64-linux-gnu/bin:$PATH
```
或者修改用户默认环境配置文件`.bashrc`和`.zshrc`，`.profile`文件也可以，需要注意是否用户登录环境
```bash
source ~/.zshrc
```

2. 检查是否生效

```bash
aarch64-linux-gnu-gcc -v
```

3. 交叉编译标准C程序

```makefile
CC = aarch64-linux-gnu-gcc
CFLAGS = -Wall -g -O0
SRC = $(shell ls *.c)
APP = $(patsubst %.c, %, $(SRC))

all: $(APP)

%: %.c
	$(CC) $(CFLAGS) -o %@ %<

install: 
	scp $(APP) root@192.168.2.122:/code_drv

clean:
	$(RM) $(APP)

.PHONY: clean
```

## 三、内核编译

内核包通常由开发板厂商提供，应为板载电路功能经由他们修改定制后，内核功能模块随之需要改变

案例开发板手册：`https://wiki.friendlyelec.com/wiki/index.php/NanoPC-T4/zh`

### 1.安装依赖

ubuntu环境可能需要安装以下依赖包

```bash
sudo apt install libssl-dev liblz4-tool gcc-multilib libncurses5-dev
```

### 2.处理内核编译配置

使用开发板厂商的配置文件
```bash
cd ~/kernel-rockchip-nanopi4-linux-v4.4.y
rm .config
cp arch/arm64/configs/nanopi4_linux_defconfig .config
```

### 3.选择性对内核配置进行修改

使用make对.config进行GUI可视化配置修改
```bash
make menuconfig
```
![](img\013.png)

### 4.创建一个方便编译的脚本

```shell
#!/bin/bash

make ARCH=arm64 nanopi4-images -j6
```

### 5.编译内核

```bash
# make distclean 清理更彻底，包含配置文件；distclean > mrproper > clean
make clean
./mymake.sh
```

### 6.烧录内核模块

在烧录软件内只选中`kernel.img`跟`resource.img`

![](img\014.png)

## 四、驱动编写与安装

为了方便索引linux源码工程，推荐使用`Source Insight4.0`代码编辑工具，现阶段vscode配置稍显麻烦

### 1.编写Makefile文件

```makefile
#指定内核源码的主目录
KERN_DIR = /home/robin/work/kernel-rockchip-nanopi4-linux-v4.4.y

#指定模块程序文件所在的目录
CUR_DIR = $(shell pwd)

all:
	make -C $(KERN_DIR) M=$(CUR_DIR) modules

clean:
	make -C $(KERN_DIR) M=$(CUR_DIR) clean

install:
	scp *.ko root@192.168.2.123:/drv_code

#指定当前目录下那些文件作为模块程序来编译
obj-m = hello_drv.o
```

### 2.编写基础代码

```c
#include <linux/module.h>
#include <linux/init.h>

// 初始化函数实现，__init告诉内核只调用一次就释放该函数
static int __init hello_init(void)
{
	printk("call hello_init");
	printk("begin run hello");
	return 0;
}
// 效果同上
static void __exit hello_exit(void)
{
	printk("call hello_exit");
}

module_init(hello_init); //入口函数
module_exit(hello_exit);  //出口函数
MODULE_LICENSE("GPL"); //声明使用开源协议
```

### 3.编译上传到开发板

```bash
make all
make install
```

### 4.在开发板安装驱动模块

```bash
# 查看内核日志等级
cat /proc/sys/kernel/printk
# 修改内核日志等级
echo 8 > /proc/sys/kernel/printk
# 安装驱动
insmod helle_drv.ko
# 查看驱动
lsmod
# 卸载驱动
rmmod helle_drv
```

### 5.面向对象版本

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/fs.h>
#include <linux/device.h>
#include <linux/slab.h>

/*
 *      udev机制创建设备文件
 *      写容错处理
 *      面向对象
 */

//声明一个hello设备的结构体(类)
typedef struct rk_hello_drv {
        unsigned int major;
        struct class *cls;
        struct device *dev;
} hello_drv;

//定义全局结构体变量
static hello_drv  *hello_device; //声明一个对象为hello_drv的指针

//声明open函数
static int hello_open(struct inode *inode, struct file *filp);

//声明close函数
static int hello_close (struct inode *inode, struct file *filp);

//定义file_operations结构体，并指定文件操作接口的函数指针
static struct file_operations fops = {
        .owner = THIS_MODULE, //避免在使用时卸载模块
        .open = hello_open,
        .release = hello_close,
};

//定义open函数
static int hello_open(struct inode *inode, struct file *filp)
{
        printk("call %s() @ %d\n", __func__, __LINE__);
        return 0;
}

//定义close函数
static int hello_close (struct inode *inode, struct file *filp)
{
        printk("call %s() @ %d\n", __func__, __LINE__);
        return 0;
}

//驱动定义初始化函数
static int __init hello_init(void)
{
        int ret;
        //申请内核内存空间
        hello_device = kmalloc(sizeof(hello_drv), GFP_KERNEL);
        if (!hello_device) {
                printk("kmalloc fail\n");
                return -ENOMEM;
        }

        //1.注册字符设备
        hello_device->major = register_chrdev(0, "hello_drv", &fops);
        if (hello_device->major < 0) {
                printk("<kernel> register_chrdev fail\n");
                ret = -EBUSY;
                goto err_register_chrdev;
        }

        printk("<kerenl> major=%d\n", hello_device->major);


        //2.创建设备类对象
        hello_device->cls = class_create(THIS_MODULE, "hello_cls");
        if (IS_ERR(hello_device->cls)) { //测试创建是否成功,成立就失败
                ret = PTR_ERR(hello_device->cls);
                goto err_class_create;
        }

        //3.创建设备对象(设备文件)
        hello_device->dev = device_create(hello_device->cls, NULL, MKDEV(hello_device->major, 0), NULL, "hello");
        if (IS_ERR(hello_device->dev)) {
                ret = PTR_ERR(hello_device->dev);
                goto err_device_create;
        }

        printk("call %s() @ %d for oo\n", __func__, __LINE__);
        return 0;

    	// 注意：清理资源释放时要遵循“先进后出”，避免造成野指针
err_device_create:
        class_destroy(hello_device->cls);

err_class_create:
        unregister_chrdev(hello_device->major, "hello_drv");

err_register_chrdev:
        kfree(hello_device);

        return ret;
}

static void __exit hello_exit(void)
{
        //释放资源，注意“先进后出”式释放资源
        device_destroy(hello_device->cls, MKDEV(hello_device->major, 0));
        class_destroy(hello_device->cls);
        unregister_chrdev(hello_device->major, "hello_drv");
        kfree(hello_device);
        printk("call %s() @ %d\n", __FUNCTION__, __LINE__);
}


//定义驱动入口
module_init(hello_init);
//定义驱动出口
module_exit(hello_exit);
//使用开源协议
MODULE_LICENSE("GPL");
```



### 6.新版创建设备的函数

**`register_chrdev_region`**：静态指定设备

**`alloc_chrdev_region`**：动态分配设备号

```c
#include <linux/fs.h>

// from：起始设备号（主+次），如 MKDEV(200, 0)
// count：要分配的连续设备号数量
// name：设备名称（在 /proc/devices 中显示）
int register_chrdev_region(dev_t from, unsigned count, const char *name);
// dev：输出参数，用于返回内核分配的起始设备号
// baseminor：起始次设备号（通常为 0）
int alloc_chrdev_region(dev_t *dev, unsigned baseminor, unsigned count, const char *name);
// 返回0 表示成功，负值表示错误（如 -EBUSY 表示设备号已被占用）。
```

`cdev`结构体

```c
struct cdev {
	struct kobject kobj;
	struct module *owner;
	const struct file_operations *ops;
	struct list_head list;
	dev_t dev;
	unsigned int count;
};
```

**`cdev_alloc`**

```c
// 作用：在内核堆上动态分配一个 struct cdev 对象，并初始化其内部字段（如 kobj）。
// 返回值：成功返回指向 cdev 的指针；失败返回 NULL。
// 特点：
//     分配的内存需由驱动负责释放（通过 cdev_del() 自动释放）。
//     不会初始化 .ops 和 .owner，需后续手动设置。
struct cdev *cdev_alloc(void);
```



示例

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/fs.h>
#include <linux/device.h>
#include <linux/slab.h>
#include <linux/gpio.h>
#include <linux/uaccess.h>
#include <linux/cdev.h>
#include <linux/interrupt.h>

/*
 * 省略重复代码
 */

//声明一个botton设备的结构体(类)
typedef struct rk_botton_drv{
	unsigned int major;   			//主设备号
	struct class *cls;				//设备类
	struct device *dev;				//设备
	unsigned int gpio_botton; 		//botton引脚GPIO0_A5
	int gpio_irq_num;				//引脚中断号
	dev_t devno;					//保存设备号
	struct cdev *cd;  				//字符设备核心
} botton_drv;

//定义全局结构体变量
static botton_drv *botton_device; //声明一个对象为botton_drv的指针

static int __init botton_init(void) 
{
	int ret;
	//申请内核内存空间
	botton_device = kmalloc(sizeof(botton_drv), GFP_KERNEL);
	if (!botton_device) {
		printk("kmalloc fail\n");
		return -ENOMEM;
	}

#ifdef C_DIV
	//1.注册字符设备
	botton_device->major = register_chrdev(0, "botton_drv", &fops);
	if (botton_device->major < 0) {
		printk("<kernel> register_chrdev fail\n");
		ret = -EBUSY;
		goto err_register_chrdev;
	}
#else
	//1.新的注册字符设备的方法
	//分配设备号
	if(alloc_chrdev_region(&botton_device->devno, 32, 1, "botton_drv"))
	{
		printk("alloc_chrdev_region fail\n");
		ret = -EBUSY;
		goto err_register_chrdev;
	}
	//解析出主设备号; 解析次设备号用MINOR(botton_device->devno)
	botton_device->major = MAJOR(botton_device->devno);
	//创建cdev对象
	botton_device->cd = cdev_alloc();
	//初始化cdev对象
	cdev_init(botton_device->cd, &fops);
	//注册cdev对象
	cdev_add(botton_device->cd, botton_device->devno, 1);
#endif
	printk("<kerenl> major=%d\n", botton_device->major);

	//2.创建设备类对象
	botton_device->cls = class_create(THIS_MODULE, "botton_cls");
	if (IS_ERR(botton_device->cls)) { //测试创建是否成功,成立就失败
		ret = PTR_ERR(botton_device->cls);
		goto err_class_create;
	}
	
	//3.创建设备对象(设备文件)
	botton_device->dev = device_create(botton_device->cls, NULL, botton_device->devno, NULL, "botton");
	if (IS_ERR(botton_device->dev)) {
		ret = PTR_ERR(botton_device->dev);
		goto err_device_create;
	}

	//4.从gpio子系统申请资源
	botton_device->gpio_botton = 5;
	if (gpio_request(botton_device->gpio_botton, "botton_irq")) {
		printk("gpio_request fail\n");
		ret = -EBUSY;
		goto err_gpio_request;
	}

	//5.获得中断号
	botton_device->gpio_irq_num = gpio_to_irq(botton_device->gpio_botton);

	//6.注册中断
	if((ret = request_irq(botton_device->gpio_irq_num, button_handler, 
		IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, "power_key_irq", NULL)))
	{
		printk("request_irq fail\n");
		ret = -EINVAL;
		goto err_request_irq;
	}

	printk("call %s() @ %d for oo init done. irq: %d\n", __func__, __LINE__, botton_device->gpio_irq_num);
	return 0;

err_request_irq:
	gpio_free(botton_device->gpio_botton);
err_gpio_request:
	device_destroy(botton_device->cls, botton_device->devno);
err_device_create:
	class_destroy(botton_device->cls);
err_class_create:
#ifdef C_DIV
    unregister_chrdev(botton_device->major, "botton_drv");
#else    
	cdev_del(botton_device->cd);
    unregister_chrdev_region(botton_device->devno, 1);  //释放设备号
#endif
err_register_chrdev:
	kfree(botton_device);

	return ret;
}
```





## 五、驱动开发常用函数和宏

### **宏定义**

#### **`__FUNCTION__`**

函数名变量



#### **`__func__`**

同上，函数名变量



#### **`__LINE__`**

行号变量

```c
printk("call %s of %d\n", __FUNCTION__, __LINE__);
```



#### **`THIS_MODULE`**

指向当前驱动模块的指针

```c
static struct file_operations fops = {
	.owner = THIS_MODULE, //避免在用时卸载模块
	.open = led_open,
	.write = led_write,
	.release = led_close,
};
```



#### **`current`**

进程描述符对象，常用于在多进程中输出`current->pid`和`current->comm`

```c
#define current get_current()

static __always_inline struct task_struct *get_current(void)
{
	unsigned long sp_el0;
	asm ("mrs %0, sp_el0" : "=r" (sp_el0));
	return (struct task_struct *)sp_el0;
}
```





### **函数**

#### **`copy_from_user`**

拷贝用户空间的数据到内核空间

```c
#include <linux/uaccess.h>

//从用户态拷贝到内核态
//    to：内核空间的目标地址
//    from：用户空间的源地址（由用户程序提供）
//    n：要拷贝的字节数
unsigned long copy_from_user(void *to, const void __user *from, unsigned long n)
```

- `__user` 是一个编译时注解（annotation），用于提示该指针指向用户空间，不具有直接访问权限。
- 两个函数返回值都是 **未成功拷贝的字节数**（即失败的字节数）：
  - 如果返回值为 0，表示**全部拷贝成功**。
  - 如果返回值 > 0，表示**部分或全部拷贝失败**（通常是因为用户空间地址无效）。


示例

```c
ssize_t my_write (struct file *filp, const char __user *buf, size_t size, loff_t *flag)
{
	int ret;
	led_device->value = 0;
	ret = copy_from_user(&led_device->value, buf, size);
	if (ret) {
		printk("copy_from_user fail\n");
		return -EFAULT;
	}
    return 0;
}
```



#### **`copy_to_user`**

**内核空间**与**用户空间**之间安全地拷贝数据

```c
#include <linux/uaccess.h>

//从内核态拷贝到用户态
//    to：用户空间的目标地址（由用户程序提供）
//    from：内核空间的源地址
//    n：要拷贝的字节数
unsigned long copy_to_user(void __user *to, const void *from, unsigned long n);
```

示例

```c
ssize_t my_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    const char *msg = "Hello from kernel!\n";
    size_t len = strlen(msg);
    if (*pos >= len)
        return 0; // EOF
    if (count > len - *pos)
        count = len - *pos;
    if (copy_to_user(buf, msg + *pos, count))
        return -EFAULT;
    *pos += count;
    return count;
}
```

**注意事项：**

- 不能在中断上下文、原子上下文（如 spinlock 保护区域）中使用，因为可能引起睡眠（页错误处理可能导致调度）。

- 验证用户指针，可使用 `access_ok()` 提前检查用户地址是否可访问（虽然 `copy_to/from_user` 内部也会检查）

  ```c
  if (!access_ok(VERIFY_READ, user_ptr, size))
      return -EFAULT;
  ```

- 这些函数涉及地址验证和页表检查，比 `memcpy` 慢，应避免在性能关键路径中频繁调用。

- 永远不要忽略返回值！否则可能导致内核使用未初始化或错误的数据。



#### **`ioctl`**






#### **`mmap`**

（Memory Map）是一种将文件或其他对象映射到进程地址空间的内存映射机制，它允许进程像访问内存一样直接访问文件内容。

```c
#include <sys/mman.h>

//addr: 建议的映射起始地址（通常设为NULL，由内核决定）
//length: 映射区域的长度
//prot: 保护标志
//    PROT_READ: 可读
//    PROT_WRITE: 可写
//    PROT_EXEC: 可执行
//    PROT_NONE: 不可访问
//flags: 映射类型和选项
//    MAP_SHARED: 共享映射（修改会写回文件）
//    MAP_PRIVATE: 私有映射（写时复制）
//    MAP_FIXED: 使用指定地址
//    MAP_ANONYMOUS: 匿名映射（不与文件关联）
//fd: 文件描述符
//offset: 文件偏移量
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset);
```

字符设备驱动示例

```c
// 字符设备驱动中的mmap实现
static int simple_mmap(struct file *filp, struct vm_area_struct *vma)
{
    struct simple_dev *dev = filp->private_data;
    unsigned long size = vma->vm_end - vma->vm_start;
    unsigned long pfn;
    // 检查映射大小
    if (size > dev->buffer_size)
        return -EINVAL;
    // 将物理地址转换为页帧号
    pfn = virt_to_phys(dev->buffer) >> PAGE_SHIFT;
    // 建立映射
    if (remap_pfn_range(vma, vma->vm_start, pfn, size, vma->vm_page_prot))
        return -EAGAIN;
    // 设置VMA操作
    vma->vm_ops = &simple_vm_ops;
    vma->vm_private_data = dev;
    return 0;
}
static const struct vm_operations_struct simple_vm_ops = {
    .open = simple_vma_open,
    .close = simple_vma_close,
    .fault = simple_vma_fault,
};
```

零拷贝示例

```c
// 使用mmap实现零拷贝文件发送
void send_file_mmap(int sockfd, const char *filename)
{
    int fd = open(filename, O_RDONLY);
    struct stat st;
    fstat(fd, &st);
    void *map = mmap(NULL, st.st_size, PROT_READ,
                     MAP_PRIVATE, fd, 0);
    
    // 直接使用sendfile或writev发送映射的内存
    write(sockfd, map, st.st_size);
    munmap(map, st.st_size);
    close(fd);
}
```

优点：

> 1. **减少数据拷贝**：避免用户空间和内核空间之间的拷贝
> 2. **随机访问**：可以直接指针访问，不需要lseek
> 3. **内存共享**：多个进程可以共享同一映射
> 4. **延迟加载**：按需分页，减少内存占用

缺点：

> 1. **内存碎片**：可能导致地址空间碎片化
> 2. **映射开销**：小文件映射可能不划算
> 3. **同步问题**：需要处理缓存一致性
> 4. **错误处理**：缺页异常可能导致性能抖动



#### **`kmalloc`**



#### **`kfree`**



#### **`kzalloc`**



#### **`devm_kmalloc`**



#### **`devm_kfree`**




### 命令

查找文本所在文件

#### **`grep`**

```bash
grep -nir "gpio0 13" rk3399*
```





## 六、基础记忆点

### 命令执行环境

`./test.sh`：在子进程环境执行脚本内容

`. test.sh`：在当前进程执行脚本内容，等效于`source test.sh`



### 文件类型

Linux 系统中的文件类型主要可以通过 `ls -l`命令输出中每行首位的那个字符来识别。主要有以下 7 种基本类型：

| 类型             | 标识符号 | 描述                                            | 常见示例                   |
| ---------------- | -------- | ----------------------------------------------- | -------------------------- |
| **普通文件**     | `-`      | 最常见的类型，包括文本、二进制程序、图片等。    | `.txt`文档, `/bin/ls`命令  |
| **目录文件**     | `d`      | 即文件夹，用于组织文件和其他目录。              | `/home`, `/etc`            |
| **符号链接文件** | `l`      | 类似 Windows 的快捷方式，指向另一个文件或目录。 | 软件版本快捷链接           |
| **字符设备文件** | `c`      | 提供无缓冲的顺序数据流访问，如键盘、终端。      | `/dev/tty`(终端)           |
| **块设备文件**   | `b`      | 提供缓冲的随机数据块访问，如硬盘、U盘。         | `/dev/sda`(硬盘)           |
| **套接字文件**   | `s`      | 用于进程之间的网络通信或本地进程间通信。        | `/var/run`目录下的某些文件 |
| **管道文件**     | `p`      | 也称为 FIFO，用于进程间的单向数据流动。         | 由 `mkfifo`命令创建        |



### 字符设备开发

驱动开发主要是字符设备开发

![](img\015.png)



### 设备号

如果要自定义设备号，在创建的时候直接传入>0的数值，0表示系统生成主设备号

```c
register_chrdev(111, "hello_drv", &fops);
```

在代码中通常使用`MKDEV`宏来构造字符设备文件的识别号

```c
// 第一个参数是注册的主设备号，第二个是自定义的次设备号
// 宏定义里面是一个将主设备号左移20位再拼接上次设备号的运算
MKDEV(major, 0)
```



- 查看内核中已经申请的设备号

```bash
cat /proc/devices
```



- 用户态创建设备文件

```bash
# mknod 设备文件路径 c|b 主设备号 次设备号
mknod /dev/hello1 c 250 0
```



### 结构体`file_operations`



### 内联函数`inline`

使用`inline`关键字建议编译器将函数调用操作替换为函数体本身的代码，这类似于一种高级的复制粘贴。它的主要目的是**消除函数调用的开销**，例如参数压栈、跳转和返回操作，这对于小型且频繁调用的函数能提升效率

但需要注意，`inline`仅仅是**给编译器的一个建议**。编译器会根据优化策略、函数复杂度和大小等因素决定是否真正内联。例如，包含循环或递归调用的复杂函数，或者函数体过大时，编译器很可能忽略内联建议

内联函数常常被拿来与宏（`#define`）比较，因为它们都能在编译前进行代码展开。但它们在本质上有显著区别，具体如下表所示：

| 特性 | 内联函数 (`inline`) | 宏定义 (`#define`) |
| :--- | :--- | :--- |
| **处理阶段** | **编译时**展开，是编译器行为 | **预编译时**进行文本替换，是预处理器行为 |
| **类型检查** | **执行严格的类型检查**，安全性高 | **不做任何类型检查**，直接替换文本，容易因参数类型或表达式复杂性导致意想不到的错误 |
| **调试支持** | 支持符号调试，调试信息更友好 | 调试困难，因为调试器看到的是替换后的代码，而非宏名本身 |
| **副作用风险** | 无副作用，参数只计算一次 | 参数可能被多次求值，容易产生副作用，例如 `#define SQUARE(x) ((x)*(x))`，若参数为`a++`，则`a`会被递增两次 |

示例

```c
static inline int register_chrdev(unsigned int major, const char *name, const struct file_operations *fops)
{
	return __register_chrdev(major, 0, 256, name, fops);
}
```



### **`file`结构体传递数据**

示例

```c
//定义write函数
static ssize_t led_write(struct file *filp, const char __user *user_buf, size_t c, loff_t *lof)
{
	/* 可以从struct file中获取用户态不同进程中的 */
	return 0;
}
```

可以从用户态携带数据

```c
struct file {
//....
	/* needed for tty driver, and maybe others */
	void			*private_data;   
//....
} 

```



### **`container_of`**





## 七、寄存器开发



**`ioremap`**：物理地址映射









## 八、GPIO子系统

自 Linux 内核 4.8 起，GPIO 子系统进行了现代化重构，引入了 **gpiod（GPIO descriptor）** 接口，取代了早期基于整数 GPIO 编号（gpio number）的旧接口

- **基于描述符的API** (推荐)：`include/linux/gpio/consumer.h`
- **传统整数API** (逐渐淘汰)：`include/linux/gpio.h`

**注意**：gpio子系统只能解决输出输入简单控制，控制寄存器复用片上外设级别需要pinctl子系统操作



### 1.基于描述符的API

#### 获取GPIO句柄

```c
#include <linux/gpio/consumer.h>

/**
 * @brief 获取单个GPIO描述符
 * @param dev: 使用该GPIO的设备指针（通常为&pdev->dev）
 * @param con_id: GPIO在设备树中的名称标识符（如"enable"、"reset"）
 * @param flags: GPIO配置标志，指定方向和初始状态
 * @return 成功返回GPIO描述符指针，失败返回ERR_PTR错误码
 * 
 * 示例：获取设备树中名为"enable"的GPIO
 * struct gpio_desc *en = gpiod_get(&pdev->dev, "enable", GPIOD_OUT_LOW);
 */
struct gpio_desc *gpiod_get(struct device *dev, const char *con_id, enum gpiod_flags flags);

/**
 * @brief 获取索引GPIO描述符（用于同名称多个GPIO的情况）
 * @param dev: 使用该GPIO的设备指针
 * @param con_id: GPIO在设备树中的名称标识符
 * @param index: GPIO索引号（从0开始）
 * @param flags: GPIO配置标志
 * @return 成功返回GPIO描述符指针，失败返回ERR_PTR错误码
 * 
 * 示例：设备树中有`cs-gpios = <&gpio1 0>, <&gpio1 1>, <&gpio1 2>;`
 * 获取第二个片选：gpiod_get_index(dev, "cs", 1, GPIOD_OUT_HIGH);
 */
struct gpio_desc *gpiod_get_index(struct device *dev, const char *con_id, unsigned int index, enum gpiod_flags flags);

/**
 * @brief 设备管理版本的GPIO获取函数（自动释放）
 * @param dev: 使用该GPIO的设备指针
 * @param con_id: GPIO在设备树中的名称标识符
 * @param flags: GPIO配置标志
 * @return 成功返回GPIO描述符指针，失败返回ERR_PTR错误码
 * 
 * 注意：使用此函数获取的GPIO会在设备释放时自动调用gpiod_put()
 * 无需在remove函数中手动释放，避免资源泄漏
 */
struct gpio_desc *devm_gpiod_get(struct device *dev, const char *con_id, enum gpiod_flags flags);
```

**常用flags：**

- `GPIOD_IN` - 输入方向
- `GPIOD_OUT_LOW` - 输出方向，初始低电平
- `GPIOD_OUT_HIGH` - 输出方向，初始高电平
- `GPIOD_ASIS` - 不改变当前方向

#### 配置GPIO

```c
/**
 * @brief 设置GPIO为输入方向
 * @param desc: GPIO描述符指针（由gpiod_get等函数获取）
 * @return 成功返回0，失败返回负的错误码
 * 
 * 错误码：
 * -EINVAL: GPIO描述符无效
 * -ENOTSUPP: GPIO不支持输入方向
 * 其他: 硬件相关的错误
 */
int gpiod_direction_input(struct gpio_desc *desc);
/**
 * @brief 设置GPIO为输出方向并指定初始值
 * @param desc: GPIO描述符指针
 * @param value: 初始输出值
 *              - 0: 输出低电平
 *              - 1: 输出高电平
 * @return 成功返回0，失败返回负的错误码
 */
int gpiod_direction_output(struct gpio_desc *desc, int value);

/**
 * @brief 读取GPIO当前值（原子上下文使用）
 * @param desc: GPIO描述符指针
 * @return GPIO的逻辑值（已考虑active-low）：
 *         - 0: 低电平
 *         - 1: 高电平
 * 
 * 注意：此函数可以在中断上下文等原子上下文中使用
 * 但不能用于可能导致休眠的GPIO控制器（如I2C GPIO扩展器）
 */
int gpiod_get_value(const struct gpio_desc *desc);

/**
 * @brief 设置GPIO输出值（原子上下文使用）
 * @param desc: GPIO描述符指针
 * @param value: 要设置的值
 *              - 0: 输出低电平
 *              - 1: 输出高电平
 */
void gpiod_set_value(struct gpio_desc *desc, int value);

// 设置多个GPIO值
void gpiod_set_array_value(unsigned int array_size, struct gpio_desc **desc_array, int *value_array);


/**
 * @brief 读取GPIO当前值（可能休眠版本）；获取带偏置的GPIO值（考虑了active_low）
 * @param desc: GPIO描述符指针
 * @return GPIO的逻辑值
 * 
 * 适用于通过I2C、SPI等总线访问的GPIO控制器
 * 这些控制器的访问可能引起休眠，不能在原子上下文使用
 */
int gpiod_get_value_cansleep(const struct gpio_desc *desc);
/**
 * @brief 设置GPIO输出值（可能休眠版本）
 * @param desc: GPIO描述符指针
 * @param value: 要设置的值
 */
void gpiod_set_value_cansleep(struct gpio_desc *desc, int value);
```

#### GPIO中断

```c
/**
 * @brief 将GPIO转换为中断号
 * @param desc: GPIO描述符指针（必须已配置为输入）
 * @return 成功返回中断号，失败返回负的错误码
 * 
 * 错误码：
 * -ENXIO: GPIO不支持中断
 * -EINVAL: GPIO描述符无效或未配置为输入
 * 
 * 示例：
 * int irq = gpiod_to_irq(button_gpio);
 * request_irq(irq, handler, IRQF_TRIGGER_RISING, "button", dev);
 */
int gpiod_to_irq(const struct gpio_desc *desc);

// 设置中断触发类型（通常在request_irq中使用）
irqreturn_t gpio_irq_handler(int irq, void *data);
```

#### 释放GPIO

```c
void gpiod_put(struct gpio_desc *desc);
void devm_gpiod_put(struct device *dev, struct gpio_desc *desc);
```





### 2.传统整数API



#### GPIO编号管理

```c
#include <linux/gpio.h>

// GPIO号与描述符转换
int desc_to_gpio(const struct gpio_desc *desc);
struct gpio_desc *gpio_to_desc(unsigned gpio);

// GPIO号有效性检查
bool gpio_is_valid(int gpio);
```



#### 申请和释放

```c
// 申请GPIO
int gpio_request(unsigned gpio, const char *label);

// 配置方向
int gpio_direction_input(unsigned gpio);
int gpio_direction_output(unsigned gpio, int value);

// 读写值
int gpio_get_value(unsigned gpio);
void gpio_set_value(unsigned gpio, int value);

// 释放GPIO
void gpio_free(unsigned gpio);
```

简单示例

```c
//初始化硬件
led_device->gpio_led = 55; //GPIO1_C7
//申请引脚
ret = gpio_request(led_device->gpio_led, "led2_green");
if (ret) {
    printk("gpio_request fail\n");
    goto err_gpio_request;
}
//设置引脚为输出功能
gpio_direction_output(led_device->gpio_led, 0); //默认led off
//释放引脚
gpio_free(led_device->gpio_led);
```





### 3.设备树示例

```c
// 示例：LED控制
leds {
    compatible = "gpio-leds";
    led1 {
        label = "system:red:status";
        gpios = <&gpio0 12 GPIO_ACTIVE_HIGH>;
        linux,default-trigger = "heartbeat";
        default-state = "off";
    };
};

// 示例：按钮输入
buttons {
    compatible = "gpio-keys";
    button1 {
        label = "Power Button";
        gpios = <&gpio0 13 GPIO_ACTIVE_LOW>;
        linux,code = <KEY_POWER>;
    };
};

// 自定义设备
my_device {
    compatible = "my-company,my-device";
    enable-gpios = <&gpio1 5 GPIO_ACTIVE_HIGH>;
    reset-gpios = <&gpio1 6 GPIO_ACTIVE_LOW>;
    interrupt-parent = <&gpio0>;
    interrupts = <14 IRQ_TYPE_EDGE_RISING>;
};
```



### 4.驱动代码示例

```c
#include <linux/module.h>
#include <linux/platform_device.h>
#include <linux/gpio/consumer.h>

struct my_device {
    struct gpio_desc *enable_gpio;
    struct gpio_desc *reset_gpio;
    int irq;
};

static irqreturn_t my_interrupt_handler(int irq, void *dev_id)
{
    struct my_device *dev = dev_id;
    int value = gpiod_get_value(dev->reset_gpio);
    
    printk(KERN_INFO "GPIO interrupt triggered, value: %d\n", value);
    return IRQ_HANDLED;
}

static int my_probe(struct platform_device *pdev)
{
    struct device *dev = &pdev->dev;
    struct my_device *my_dev;
    int ret;
    
    // 分配设备结构
    my_dev = devm_kzalloc(dev, sizeof(*my_dev), GFP_KERNEL);
    if (!my_dev)
        return -ENOMEM;
    
    // 获取GPIO描述符
    my_dev->enable_gpio = devm_gpiod_get(dev, "enable", GPIOD_OUT_LOW);
    if (IS_ERR(my_dev->enable_gpio))
        return dev_err_probe(dev, PTR_ERR(my_dev->enable_gpio),
                           "failed to get enable GPIO\n");
    
    my_dev->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_IN);
    if (IS_ERR(my_dev->reset_gpio))
        return dev_err_probe(dev, PTR_ERR(my_dev->reset_gpio),
                           "failed to get reset GPIO\n");
    
    // 控制GPIO
    gpiod_set_value(my_dev->enable_gpio, 1);
    mdelay(10);
    
    // 配置为中断
    my_dev->irq = gpiod_to_irq(my_dev->reset_gpio);
    ret = devm_request_irq(dev, my_dev->irq, my_interrupt_handler,
                          IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
                          "my-device", my_dev);
    if (ret) {
        dev_err(dev, "failed to request IRQ: %d\n", ret);
        return ret;
    }
    
    // 存储设备数据
    platform_set_drvdata(pdev, my_dev);
    
    dev_info(dev, "device probed successfully\n");
    return 0;
}

static int my_remove(struct platform_device *pdev)
{
    struct my_device *my_dev = platform_get_drvdata(pdev);
    
    // 关闭设备
    gpiod_set_value(my_dev->enable_gpio, 0);
    
    dev_info(&pdev->dev, "device removed\n");
    return 0;
}

static const struct of_device_id my_of_match[] = {
    { .compatible = "my-company,my-device" },
    {},
};
MODULE_DEVICE_TABLE(of, my_of_match);

static struct platform_driver my_driver = {
    .driver = {
        .name = "my-device",
        .of_match_table = my_of_match,
    },
    .probe = my_probe,
    .remove = my_remove,
};
module_platform_driver(my_driver);
```



### 5.常用调试命令

```bash
# 查看GPIO使用情况
$ cat /sys/kernel/debug/gpio

# GPIO sysfs接口（如果配置了CONFIG_GPIO_SYSFS）
$ ls /sys/class/gpio/
export  gpiochip0  gpiochip32  unexport

# 导出GPIO进行手动控制
$ echo 12 > /sys/class/gpio/export
$ echo out > /sys/class/gpio/gpio12/direction
$ echo 1 > /sys/class/gpio/gpio12/value
```



### 6.内核配置选项

```bash
# 内核配置选项
CONFIG_GPIOLIB=y           # 启用GPIO库支持
CONFIG_GPIO_SYSFS=y        # 启用sysfs接口（调试用）
CONFIG_GPIO_CDEV=y         # 启用字符设备接口（推荐）
CONFIG_GPIO_DEBUGFS=y      # 启用debugfs接口
```



## 九、中断子系统

1. **硬件触发中断**（如 GPIO、UART、Timer 等）。
2. **CPU 跳转到内核的中断入口**。
3. **通用 IRQ 层调用注册的中断处理函数（handler）**。
4. **驱动在中断上下文中执行关键、快速的操作（上半部）**。
5. **如有耗时工作，提交到下半部（如 workqueue、tasklet、softirq）处理**。



### 1.中断注册与释放函数

查看已注册中断号：`cat /proc/interrupts`

#### **`request_irq`**

注册中断处理函数（最常用）

```c
/*
 * irq：中断号（可从 GPIO、设备树等获取）。
 * handler：中断处理函数，原型为：irqreturn_t handler(int irq, void *dev_id);
 * flags：中断标志（见下表）。
 * name：中断名称（/proc/interrupts 中可见）。
 * dev：设备私有数据指针（用于共享中断时区分设备）。
 */
int request_irq(unsigned int irq, irq_handler_t handler, unsigned long flags, const char *name, void *dev);
/* 返回值：0 成功，负值表示错误（如 -EBUSY 表示 IRQ 被占用且未共享）。 */
```

常用 flags：

| `IRQF_SHARED`          | 共享中断线（多个设备共用同一 IRQ） |
| ---------------------- | ---------------------------------- |
| `IRQF_TRIGGER_RISING`  | 上升沿触发（常用于 GPIO）          |
| `IRQF_TRIGGER_FALLING` | 下降沿触发                         |
| `IRQF_TRIGGER_HIGH`    | 高电平触发                         |
| `IRQF_TRIGGER_LOW`     | 低电平触发                         |
| `IRQF_ONESHOT`         | 线程化中断（见下文）               |



#### **`free_irq`**

释放中断

```c
/*
 * 必须使用与 request_irq() 相同的 irq 和 dev 参数。
 * 若为共享中断，仅释放当前设备的 handler；当所有 handler 释放后，IRQ 才真正释放。
 */
void free_irq(unsigned int irq, void *dev);
```



### 2.从 GPIO 获取 IRQ 号



#### **`gpiod_to_irq`** 

```c
int irq = gpiod_to_irq(gpio_desc);
if (irq < 0) {
    dev_err(dev, "GPIO to IRQ failed\n");
    return irq;
}
```



#### **`gpio_to_irq`**

```c
/* 老版本gpio子系统的整数引脚号 */
int irq = gpio_to_irq(unsigned int gpio);
/* 返回中断号 */
```



### 3.设备树中的中断描述

```dtd
my_device: my-device@0 {
    compatible = "example,mydev";
    interrupts = <GIC_SPI 35 IRQ_TYPE_LEVEL_HIGH>;
    interrupt-parent = <&gic>;
};
```

代码中获取中断号

```c
/* platform_get_irq() 自动解析设备树 interrupts 属性。 */
int irq = platform_get_irq(pdev, 0); // 获取第 0 个中断
if (irq < 0)
    return irq;
```

其他常用函数

| 函数                          | 说明                                                |
| ----------------------------- | --------------------------------------------------- |
| `enable_irq(irq)`             | 启用已注册的中断（通常与 `disable_irq()` 配对）     |
| `disable_irq(irq)`            | 禁用中断（会等待当前 handler 执行完，**可能睡眠**） |
| `disable_irq_nosync(irq)`     | 禁用中断但不等待（**可在中断上下文调用**）          |
| `synchronize_irq(irq)`        | 等待该 IRQ 所有 pending handler 执行完毕            |
| `irq_set_irq_type(irq, type)` | 动态修改触发类型（如从上升沿改为下降沿）            |

> **注意**：`disable_irq()` 会阻塞直到当前中断处理完成，**不能在中断上下文调用**；若需在中断中禁用自身，应使用 `disable_irq_nosync()`。



### 4.中断下半部

内核服务，相当于内核线程

- `[ksoftirqd]`：内核小任务
  - `ps -aux | grep ksoftirqd`

- `[kworker]`：工作队列，进程上下文
  - `ps -aux | grep kworker`


| 特性           | `workqueue`                           | `tasklet`                           |
| -------------- | ------------------------------------- | ----------------------------------- |
| **执行上下文** | 进程上下文（内核线程）                | 软中断上下文（中断上下文）          |
| **是否可睡眠** | ✅ 可以（`GFP_KERNEL`、`msleep()` 等） | ❌ 不可睡眠                          |
| **执行时机**   | 异步，由内核线程调度                  | 紧接在硬中断之后（`irq_exit()` 时） |
| **能否被抢占** | ✅ 可被调度器抢占                      | ❌ 不可抢占（关中断执行）            |
| **SMP 支持**   | ✅ 可在任意 CPU 执行（默认）           | ✅ 但默认绑定到触发中断的 CPU        |
| **重复调度**   | 可多次调度（每次创建新 work）         | 同一 tasklet 在执行前不会重复运行   |
| **延迟敏感性** | 较低（有调度延迟）                    | 较高（延迟小）                      |
| **推荐使用**   | **现代驱动首选**                      | 仅用于简单、快速、非阻塞任务        |

> 📌 **结论**：**除非有极低延迟要求，否则优先使用 `workqueue`**。`tasklet` 已逐渐被弃用（内核社区建议迁移到 `workqueue` 或 `irq` 线程）。



####  `workqueue`基本概念

- `workqueue` 是一个**内核线程池**，用于异步执行“工作项”（`struct work_struct`）。
- 工作项被提交后，由内核线程（如 `kworker/0:1`）在**进程上下文**中执行。
- 可睡眠、可调度、可使用 `mutex`、`wait_queue` 等。

#### 常用 API

方式一：静态定义（推荐）

```c
#include <linux/workqueue.h>

void my_work_handler(struct work_struct *work)
{
    // 可在此处 sleep、分配内存（GFP_KERNEL）、调用 I2C 等
    msleep(100);
    dev_info(my_dev, "Work done!\n");
}

DECLARE_WORK(my_work, my_work_handler);
```

方式二：动态初始化（用于嵌入结构体）

```c
struct my_device {
    struct work_struct work_name;
    int data;
};

void my_work_handler(struct work_struct *pwork)
{
    //container_of拿到驱动中共享的数据进行后续逻辑
    struct my_device *mydev = container_of(pwork, struct my_device, work_name);
    // 使用 mydev->data
}

// 初始化
INIT_WORK(&mydev->work, my_work_handler);
```



#### 调度工作项

```c
schedule_work(&my_work);          // 提交到系统默认 workqueue（system_wq）
```

> 系统默认 workqueue 是 `system_wq`（绑定 CPU，支持并发）。

#### 带延迟调度（定时执行）

```c
struct delayed_work my_delayed_work;

// 初始化
INIT_DELAYED_WORK(&my_delayed_work, my_delayed_handler);

// 延迟 2 秒后执行
schedule_delayed_work(&my_delayed_work, msecs_to_jiffies(2000));

// 取消延迟工作（若尚未执行）
cancel_delayed_work(&my_delayed_work);
// 或安全取消（等待执行完）
cancel_delayed_work_sync(&my_delayed_work);
```

#### 使用自定义 workqueue（高级）

```c
struct workqueue_struct *my_wq;

// 创建单线程 workqueue（按需）
my_wq = alloc_workqueue("my_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);

// 调度到自定义队列
queue_work(my_wq, &my_work);

// 销毁
destroy_workqueue(my_wq);
```

> 一般驱动无需自定义，使用 `system_wq` 即可。

**完整示例**

```c
#include <linux/module.h>
#include <linux/workqueue.h>
#include <linux/delay.h>

static struct work_struct my_work;

static void my_work_handler(struct work_struct *work)
{
    printk(KERN_INFO "Work running in process context (PID %d)\n", current->pid);
    msleep(1000);  // ✅ 合法：可睡眠
    printk(KERN_INFO "Work done.\n");
}

static int __init work_init(void)
{
    INIT_WORK(&my_work, my_work_handler);
    schedule_work(&my_work);  // 提交工作
    return 0;
}

static void __exit work_exit(void)
{
    // 确保 work 已执行完
    flush_scheduled_work();  // 等待 system_wq 中所有 work 完成
}

module_init(work_init);
module_exit(work_exit);
MODULE_LICENSE("GPL");
```

> `flush_scheduled_work()` 已弃用，推荐使用 `flush_work(&my_work)`（针对特定 work）。





#### Tasklet 详解

- `tasklet` 是一种**软中断（softirq）** 机制，在**中断上下文**执行。
- **不可睡眠、不可阻塞**，执行期间**本地 CPU 中断被关闭**。
- 同一 `tasklet` 在执行前不会被重复调度（自动去重）。

> ⚠️ **注意**：自 Linux 5.0 起，`tasklet` 被标记为 **deprecated**（不推荐新代码使用）。



#### 常用 API

静态定义：

```c
void my_tasklet_handler(unsigned long data)
{
    // ❌ 不能 sleep！
    // ✅ 可快速操作硬件、唤醒进程等
    printk("Tasklet running on CPU %d\n", smp_processor_id());
}

DECLARE_TASKLET(my_tasklet, my_tasklet_handler, (unsigned long)my_dev);
```

动态初始化：

```c
struct tasklet_struct my_tasklet;

void my_tasklet_handler(unsigned long data)
{
    struct my_device *dev = (struct my_device *)data;
    // 处理
}

// 初始化
tasklet_init(&my_tasklet, my_tasklet_handler, (unsigned long)my_dev);//可以地址传参
```

#### 调度 tasklet

```c
tasklet_schedule(&my_tasklet);  // 标记为待执行
```

> 多次调用 `tasklet_schedule()` 不会重复执行（直到当前执行完成）。

#### 禁用/启用 tasklet

```c
tasklet_disable(&my_tasklet);  // 禁用（可嵌套）
tasklet_enable(&my_tasklet);   // 启用
```

#### 销毁 tasklet（模块退出时）

```c
tasklet_kill(&my_tasklet);  // 等待其执行完并禁用
```

> 必须调用 `tasklet_kill()`，否则可能在模块卸载后执行，导致崩溃。



**Tasklet 示例（仅用于理解，不推荐新项目使用）**

```c
#include <linux/module.h>
#include <linux/interrupt.h>

static struct tasklet_struct my_tasklet;

static void my_tasklet_handler(unsigned long data)
{
    printk(KERN_INFO "Tasklet: cannot sleep! CPU%d\n", smp_processor_id());
    // 不能调用 msleep(), kmalloc(GFP_KERNEL) 等
}

static int __init tasklet_init(void)
{
    tasklet_init(&my_tasklet, my_tasklet_handler, 0);
    tasklet_schedule(&my_tasklet);
    return 0;
}

static void __exit tasklet_exit(void)
{
    tasklet_kill(&my_tasklet);  // 必须！
}

module_init(tasklet_init);
module_exit(tasklet_exit);
MODULE_LICENSE("GPL");
```



| 场景                              | 推荐机制                                           |
| --------------------------------- | -------------------------------------------------- |
| 需要睡眠（如 I2C、USB、文件操作） | ✅ `workqueue`                                      |
| 需要分配内存（`GFP_KERNEL`）      | ✅ `workqueue`                                      |
| 简单数据处理、唤醒等待队列        | ✅ `workqueue`                                      |
| 极低延迟（微秒级），且逻辑极简单  | ⚠️ 可考虑 `tasklet`（但优先考虑线程化中断）         |
| 新驱动开发                        | ❌ 避免 `tasklet`，用 `workqueue` 或 `threaded IRQ` |

> 🔧 **现代替代方案**：
> 对于需要“快速响应 + 可睡眠”的场景，**线程化中断（`request_threaded_irq()`）** 是更优选择，无需手动调度下半部。





### 5.线程化中断

推荐用于复杂处理

当需要在中断中执行可能阻塞的操作（如 I2C 通信、文件操作等），应使用 **线程化中断**。它将中断分为两部分：

- **上半部（hardirq）**：极简，仅唤醒内核线程。
- **下半部（thread_fn）**：在内核线程中运行，可睡眠。



#### **`request_threaded_irq`**

```c
/*
 * handler：可为 NULL（表示默认只做 ACK 和唤醒线程）。
 * thread_fn：实际处理函数（可睡眠）。
 * 若 handler 非 NULL，需返回 IRQ_WAKE_THREAD 才会调用 thread_fn。
 */
int request_threaded_irq(unsigned int irq, irq_handler_t handler, irq_handler_t thread_fn,
                         unsigned long flags, const char *name, void *dev);
```

示例

```c
static irqreturn_t my_irq_handler(int irq, void *dev)
{
    // 快速判断是否是本设备的中断（如读状态寄存器）
    if (!is_my_device_interrupt())
        return IRQ_NONE;
    return IRQ_WAKE_THREAD; // 唤醒线程
}

static irqreturn_t my_irq_thread_fn(int irq, void *dev)
{
    // 可在此处 sleep、调用 I2C、printk 等
    msleep(10);  // ✅ 合法
    i2c_smbus_read_byte(...);
    return IRQ_HANDLED;
}

// 注册
request_threaded_irq(irq, my_irq_handler, my_irq_thread_fn,
                     IRQF_ONESHOT | IRQF_TRIGGER_FALLING,
                     "my-device", my_dev);
```

> **关键点**：
>
> - 必须指定 `IRQF_ONESHOT`：表示中断线在 thread_fn 完成前保持关闭，防止嵌套。
> - 若 `handler` 为 `NULL`，内核自动处理 ACK 并唤醒线程。





## 十、休眠与唤醒

`init_waitqueue_head(wait_queue_head_t  *wq)`





## 十一、平台总线驱动模型

平台总线（Platform Bus）是 Linux 内核中一种重要的虚拟总线机制，用于管理那些**不通过物理总线（如 PCI、USB、I2C、SPI 等）连接到系统的设备**。这些设备通常是 SoC（片上系统）内部集成的外设（如 UART、GPIO、定时器、看门狗、PWM、DMA 控制器等），或者是系统中“固定存在”的硬件资源。



### **平台总线的三要素**

1. **平台设备（platform_device）**
   - 描述硬件信息：如内存地址、中断号、DMA 通道、时钟、引脚复用等。
   - 由设备树（Device Tree）或板级文件（旧版）静态定义。
2. **平台驱动（platform_driver）**
   - 实现设备操作逻辑：probe、remove、suspend、resume 等。
   - 通过 `name` 或 `of_match_table` 与设备匹配。
3. **平台总线（platform_bus）**
   - 虚拟总线，负责设备与驱动的匹配和生命周期管理。



### 平台设备与驱动的匹配机制

按优先级

1. **设备树匹配（主流方式，现代 SoC 基本都用）**

```dtd
my_device: my-device@12340000 {
    compatible = "vendor,my-device";
    reg = <0x12340000 0x1000>;  // 寄存器基地址和大小
    interrupts = <0 25 4>;       // 中断号
    clocks = <&clkc CLK_UART>;
    status = "okay";
};
```



```c
static const struct of_device_id my_driver_of_match[] = {
    { .compatible = "vendor,my-device" },
    { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, my_driver_of_match);

static struct platform_driver my_driver = {
    .probe = my_probe,
    .remove = my_remove,
    .driver = {
        .name = "my-device",
        .of_match_table = my_driver_of_match,
    },
};
```

2. **ACPI 匹配（用于 x86 平台）**
3. **ID 表匹配（`id_table`，较少用）**
4. **name 字符串匹配（最老方式，不推荐）**

> 设备树匹配优先级最高。一旦定义了 `of_match_table`，内核会优先使用设备树匹配，忽略 `.name`。



### 1.平台设备

示例代码

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/platform_device.h>
#include "led.h"

//创建3个引脚对象
static struct rk_gpio_led rk_leds[] = {
	[0] = {
		.gpio = 13,
		.desc = "gpio_led1",
	},
};

//创建自定义平台数据对象
static struct leds_platform_data rk_led_platform_data = {
	.leds = rk_leds,
	.nleds = ARRAY_SIZE(rk_leds),
};


//定义资源对象，物理地址信息，使用时进行ioremap操作
static struct resource led_resources[] = {
	[0] = {
		.start	= CRU_CLKGATE_CON31, //起始地址
		.end	= CRU_CLKGATE_CON31 + SIZE_4 - 1, //结束地址
		.flags	= IORESOURCE_MEM, //地址类型
	},
	[1] = {
		.start	= GRF_GPIO2A_IOMUX,
		.end	= GRF_GPIO2A_IOMUX + SIZE_4 - 1,
		.flags	= IORESOURCE_MEM, //地址类型
	},
	[2] = {
		.start	= 75,
		.end	= 75,
		.flags	= IORESOURCE_IRQ, //中断类型
	},
	
	[3] = {
		.start	= GPIO2_SWPORTA_DDR,
		.end	= GPIO2_SWPORTA_DDR + SIZE_4 - 1,
		.flags	= IORESOURCE_MEM, //地址类型
	},
	
	[4] = {
		.start	= GPIO2_SWPORTA_DR,
		.end	= GPIO2_SWPORTA_DR + SIZE_4 - 1,
		.flags	= IORESOURCE_MEM, //地址类型
	},
};

//设备推出的函数
static void led_release(struct device *dev)
{
	printk("%s run device >%s >%d\n",current->comm, __func__, __LINE__);
}


/*
 * 定义平台设备对象
 */
static struct platform_device led_device = {
	.name = "rk_led1",   							// 平台设备对象的名称
	.id = -1,
	.dev = {
		.release = led_release, 					//设备注销时需要的函数
		.platform_data = &rk_led_platform_data, 	//自定义的设备数据
	},
	.resource = led_resources, 						// 资源组
	.num_resources = ARRAY_SIZE(led_resources), 	// 资源组长度
};

//入口
static int __init dev_led_init(void)
{
	printk("%s run >%s >%d\n",current->comm, __func__, __LINE__);
	//注册平台设备
	return platform_device_register(&led_device);
}

//出口
static void __exit dev_led_exit(void)
{
	printk("%s run device >%s >%d\n",current->comm, __func__, __LINE__);
	//注销平台设备
	platform_device_unregister(&led_device);
}


module_init(dev_led_init);
module_exit(dev_led_exit);
MODULE_LICENSE("GPL");
```



#### 调试

装载平台设备后进入系统目录查看

```bash
root@NanoPC-T4:~# ll /sys/bus/platform/devices/rk_led1/
drwxr-xr-x   3 root root    0 Dec 14 08:05 ./
drwxr-xr-x 154 root root    0 Dec 14 07:27 ../
lrwxrwxrwx   1 root root    0 Dec 14 08:05 driver -> ../../../bus/platform/drivers/rk_led/ # 匹配到的驱动
-rw-r--r--   1 root root 4096 Dec 14 08:05 driver_override
-r--r--r--   1 root root 4096 Dec 14 08:05 modalias
drwxr-xr-x   2 root root    0 Dec 14 08:05 power/
lrwxrwxrwx   1 root root    0 Dec 14 08:05 subsystem -> ../../../bus/platform/
-rw-r--r--   1 root root 4096 Dec 14 07:33 uevent

```





### 2.平台驱动

示例代码

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/platform_device.h>
#include <linux/fs.h>
#include <linux/device.h>
#include <linux/slab.h>
#include <linux/gpio.h>
#include <linux/uaccess.h>
#include "led.h"

//定义一个led设备的结构体(类)
struct rk_led {
	unsigned int major;//主设备号
	struct class *cls;//设备类
	struct device *dev;//字符设备
	int gpio_led; //保存引脚号
	int value; //保存on值
};

static struct rk_led  *led_device; //声明一个对象为rk_led的指针

static int led_open(struct inode *inode, struct file *filp)
{
	printk("%s run driver >%s >%d\n",current->comm, __func__, __LINE__);
	return 0;
}

static int led_close (struct inode *inode, struct file *filp)
{
	gpio_direction_output(led_device->gpio_led, 0); //led off
	printk("%s run driver >%s >%d\n",current->comm, __func__, __LINE__);
	return 0;
}
static ssize_t led_write (struct file *filp, const char __user *buf, size_t size, loff_t *flag)
{
	int ret;
	led_device->value = 0;
	ret = copy_from_user(&led_device->value, buf, size);
	if (ret) {
		printk("copy_from_user fail\n");
		return -EFAULT;
	}
	printk("<kernel> write: %d\n", led_device->value);
	if (led_device->value) {
		printk("led on \n");
		gpio_direction_output(led_device->gpio_led, 1); //led on
	} else {
		printk("led off \n");
		gpio_direction_output(led_device->gpio_led, 0); //led off
	}
	printk("%s run driver >%s >%d\n",current->comm, __func__, __LINE__);
	return 0;
}

static struct file_operations fops = {
	.owner = THIS_MODULE, //避免在用时卸载模块
	.open = led_open,
	.write = led_write,
	.release = led_close,
}; //定义文件操作接口 open ,close



//当平台设备和平台驱动匹配时会调用probe函数
//参数pdev就是匹配上的平台设备对象
//在这里创建设备文件
static int led_probe(struct platform_device *pdev)
{
	int ret;
	//1.拿到设备传过来的自定义数据
	struct leds_platform_data *pdata = (struct leds_platform_data *)pdev->dev.platform_data;
	led_device = kmalloc(sizeof(struct rk_led), GFP_KERNEL);
	if (!led_device) {
		printk("kmalloc fail\n");
		return -ENOMEM;
	}
	//2.注册字符设备对象
	led_device->major = register_chrdev(0, "led_drv", &fops);
	if (led_device->major < 0) {
		printk("register_chrdev fail\n");
		ret = -EBUSY;
		goto err_register_chrdev;
	}
	printk("%s run driver >%s >%d major=%d\n", current->comm, __func__, __LINE__, led_device->major);
	//3.创建设备类对象
	led_device->cls = class_create(THIS_MODULE, "led_cls");
	if (IS_ERR(led_device->cls)) { //测试创建是否成功,成立就失败
		ret = PTR_ERR(led_device->cls);
		goto err_class_create;
	}
	//4.创建设备对象(设备文件)
	led_device->dev = device_create(led_device->cls, NULL, MKDEV(led_device->major, 32), NULL, "led");
	if (IS_ERR(led_device->dev)) {
		ret = PTR_ERR(led_device->dev);
		goto err_device_create;
	}
	//5.初始化硬件
	led_device->gpio_led = pdata->leds[0].gpio; //GPIO0_B5
	//6.申请引脚
	ret = gpio_request(led_device->gpio_led, pdata->leds[0].desc);
	if (ret) {
		printk("gpio_request fail\n");
		goto err_gpio_request;
	}
	//设置引脚为输出功能
	gpio_direction_output(led_device->gpio_led, 0); //默认led off
	//释放引脚
	gpio_free(led_device->gpio_led);
	printk("%s run driver >%s >%d register done.\n", current->comm, __func__, __LINE__);
	return 0;

err_gpio_request:
	device_destroy(led_device->cls, MKDEV(led_device->major, 32));
err_device_create:
	class_destroy(led_device->cls);
err_class_create:
	unregister_chrdev(led_device->major, "led_drv");
err_register_chrdev:
	kfree(led_device);
	return ret;
}

//当平台设备和平台驱动分离时会调用remove函数
static int led_remove(struct platform_device *pdev)
{
	//先申请后释放
	device_destroy(led_device->cls, MKDEV(led_device->major, 32));
	class_destroy(led_device->cls);
	unregister_chrdev(led_device->major, "led_drv");
	kfree(led_device);
	printk("%s run driver >%s >%d\n",current->comm, __func__, __LINE__);
	return 0;
}

//定义平台设备ID，可以用来匹配同类型设备，一驱动多设备
static struct platform_device_id led_device_id_table[] = {
	{ .name = "rk_led1", .driver_data = 0x1 },
	{ .name = "rk_led2", .driver_data = 0x2 },
	{ .name = "rk_led3", .driver_data = 0x3 },
	{ .name = "rk_led4", .driver_data = 0x4 },
    {},
};

//平台驱动对象
static struct platform_driver led_driver = {
	.probe	= led_probe,  				//匹配到设备自动调用
	.remove	= led_remove,  				//设备释放时自动
	.driver	= {
		.name = "rk_led", 				//平台驱动名称,又是匹配条件
	},
	.id_table = led_device_id_table,    //当id_table被定义时，优先使用这个属性里的名称匹配设备
};

//出口
static int __init drv_mod_init(void) 
{
	printk("%s run driver >%s >%d\n",current->comm, __func__, __LINE__);
	return platform_driver_register(&led_driver);
}

//入口
static void __exit drv_mod_exit(void) 
{
	printk("%s run driver >%s >%d\n",current->comm, __func__, __LINE__);
	platform_driver_unregister(&led_driver);
}

module_init(drv_mod_init);
module_exit(drv_mod_exit);
MODULE_LICENSE("GPL"); 
```

**注意**：`id_table`匹配优先于`driver.name`



#### 调试

```bash
root@NanoPC-T4:~# ll /sys/bus/platform/drivers/rk_led/
total 0
drwxr-xr-x   2 root root    0 Dec 14 08:10 ./
drwxr-xr-x 120 root root    0 Dec 14 07:27 ../
--w-------   1 root root 4096 Dec 14 08:10 bind
lrwxrwxrwx   1 root root    0 Dec 14 08:10 module -> ../../../../module/led_drv/
lrwxrwxrwx   1 root root    0 Dec 14 08:10 rk_led1 -> ../../../../devices/platform/rk_led1/  #匹配到的设备
--w-------   1 root root 4096 Dec 14 07:33 uevent
--w-------   1 root root 4096 Dec 14 08:10 unbind
```

内核日志

```bash
dmesg | grep my-device
```





#### 传递数据给驱动

- 从resources属性传递

```c
static int led_probe(struct platform_device *pdev)
{
	int i;
	int irqno;
	struct resource *addr_res; //地址资源
	struct resource *irq_res; //中断资源
	for (i = 0 ; i < 4; i++) {
		addr_res = platform_get_resource(pdev, IORESOURCE_MEM, i);   //使用这个宏从平台设备过滤出特定类型数据
		printk("<kernel> addr_res->star=0x%llx end=0x%llx\n", addr_res->start, addr_res->end);
	}

	irq_res = platform_get_resource(pdev, IORESOURCE_IRQ, 0);   //过滤特定类型数据
	printk("<kernel> irq_res->star=%lld end=%lld\n", irq_res->start, irq_res->end);
    //从平台设备中直接拿中断号；参数1：平台设备  参数2：第几个资源   失败返回-1
	irqno = platform_get_irq(pdev, 0);
	printk("<kernel> irqno=%d\n", irqno);
	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;
}
```

- 从platform_data传递自定义类型数据

```c
static int led_probe(struct platform_device *pdev)
{
	int ret = 0;
    //使用对象指针强转
	struct leds_platform_data *pdata = (struct leds_platform_data *)pdev->dev.platform_data;
   	
    return ret;
}
```



### 3.其他常用API

| 函数 | 说明 |
|------|------|
| `platform_get_resource()` | 获取内存、IO、IRQ 等资源 |
| `devm_ioremap_resource()` | 映射寄存器（自动释放） |
| `platform_get_irq()` | 获取中断号 |
| `devm_clk_get()` | 获取时钟（自动管理） |
| `devm_reset_control_get()` | 获取复位控制器 |
| `of_property_read_u32()` | 读取设备树属性 |



**注意事项**

- 必须使用 devm_ 系列函数

  - 使用 **`devm_ioremap_resource()`**、**`devm_clk_get()`** 等，确保资源在驱动卸载时自动释放。

  - 避免内存泄漏和资源冲突。

- 错误处理

  - probe 函数返回负错误码（如 `-ENOMEM`, `-EINVAL`），不要返回 0 以外的成功值。

  - 资源获取失败必须返回错误，避免后续使用空指针。





## 十二、设备树



编译设备树：`make dtbs`

```bash
robin@ubuntu:~/work/kernel-rockchip-nanopi4-linux-v4.4.y$ make dtbs
  CHK     include/config/kernel.release
  CHK     include/generated/uapi/linux/version.h
...
  VDSOSYM include/generated/vdso-offsets.h
  HOSTCC  scripts/dtc/dtc.o
...
  HOSTCC  scripts/sortextable
  HOSTCC  scripts/asn1_compiler
  HOSTCC  scripts/extract-cert
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev00.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev01.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev04.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev06.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev07.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev21.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3399-nanopi4-rev22.dtb
  DTC     arch/arm64/boot/dts/rockchip/rk3328-nanopi-r2-rev00.dtb
```



编译安装后的位置

dtsi + dts -> dtb

uboot -> dtb -> fdt  -> 内核解析fdt生成文件目录`/sys/firmware/devicetree`

```c
ll /sys/firmware
```



反编译dtb文件

```bash
dtc -I fs -O dts /sys/firmware/devicetree/base > system.dts
```





### 基本语法

```c
/ {
    model = "My Board";
    compatible = "mycompany,myboard";

    chosen {
        bootargs = "console=ttyS0,115200";
    };

    soc {
        compatible = "simple-bus";
        #address-cells = <1>;
        #size-cells = <1>;
        ranges;

        my_peripheral@10000000 {
            compatible = "mycompany,my-device";
            reg = <0x10000000 0x1000>;
            interrupts = <0 25 4>;
            clocks = <&clk 12>;
            status = "okay";
        };
    };
};
```


关键属性说明

| 属性名                           | 说明                                                         |
| -------------------------------- | ------------------------------------------------------------ |
| `compatible`                     | 驱动匹配的关键字段，格式为 `"manufacturer,model"`，可多个    |
| `reg`                            | 寄存器地址和长度，取决于父节点的 `#address-cells` / `#size-cells` |
| `interrupts`                     | 中断描述，格式由 `interrupt-parent` 和中断控制器决定         |
| `clocks` / `clock-names`         | 时钟源引用（通过 phandle）                                   |
| `status`                         | `"okay"` 表示启用，`"disabled"` 表示禁用                     |
| `#address-cells` / `#size-cells` | 定义子节点地址/大小字段的 cell 数量（通常为 1 或 2）。地址用2个32位表示64位 |



### 引用

- 使用 `&label` 引用其他节点（label 通过 `node: node_name` 声明）
- 使用 `<&node>` 在属性中引用其他节点（如 clocks、interrupt-parent）

```dtd
clk: clock-controller@100 {
    compatible = "mycompany,clk";
    reg = <0x100 0x10>;
};

my_device {
    compatible = "mycompany,my-device";
    clocks = <&clk>;
};
```



### 包含与覆盖

- 使用 `#include "file.dtsi"` 引入公共设备树片段
- 使用 **Device Tree Overlay**（.dtbo）动态修改设备树（常用于 FPGA、动态加载外设）



### 驱动匹配方式

- 使用 `of_device_id` 表与设备树 `compatible` 字段匹配
  - `of`表示`open firmware`，一般用这个开头的都是设备树相关

1. 自动匹配函数

```c
static const struct of_device_id my_driver_of_match[] = {
    { .compatible = "mycompany,my-device", },
    { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, my_driver_of_match);
```

2. `platform_driver`注册

```c
static struct platform_driver my_driver = {
    .probe = my_probe,
    .remove = my_remove,
    .driver = {
        .name = "my-device",
        .of_match_table = my_driver_of_match,
        .owner = THIS_MODULE,
    },
};

module_platform_driver(my_driver);
```



### 获取设备树资源


在 `probe()` 函数中通过 `struct device_node *np = pdev->dev.of_node;` 获取节点。

- **寄存器映射**：
  
  ```c
  void __iomem *base = devm_platform_ioremap_resource(pdev, 0);
  ```
  或
  ```c
  struct resource *res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
  void __iomem *base = devm_ioremap_resource(&pdev->dev, res);
  ```
  
- **中断获取**：
  ```c
  int irq = platform_get_irq(pdev, 0);
  ```

- **时钟获取**：
  ```c
  struct clk *clk = devm_clk_get(&pdev->dev, NULL); // 或指定 clock-name
  clk_prepare_enable(clk);
  ```

- **GPIO 获取**：
  
  ```c
  int gpio = devm_gpiod_get(&pdev->dev, "reset", GPIOD_OUT_LOW);
  ```
  
- **属性读取**：
  ```c
  u32 val;
  of_property_read_u32(np, "mycompany,timeout-ms", &val);
  ```



### 资源管理

推荐使用 `devm_*`

使用 `devm_*` 系列函数自动释放资源（如 `devm_ioremap`, `devm_clk_get`, `devm_gpiod_get`），避免忘记释放。



### 注意事项

#### 1. 设备树编写注意事项

- **compatible 字段必须准确**：是驱动匹配的唯一依据。
- **地址空间描述一致**：确保 `#address-cells` / `#size-cells` 与子节点 `reg` 格式匹配。
- **避免硬编码地址**：应通过设备树传递，而不是在驱动中写死。
- **status 控制设备启用/禁用**：调试时可临时设为 `disabled`。
- **使用 label 简化引用**：提高可读性和维护性。

#### 2. 驱动开发注意事项

- **不要在 probe 中做耗时操作**：避免阻塞系统启动。
- **错误处理要完整**：资源申请失败应 goto 或 return 错误码，并释放已申请资源。
- **使用 devm_ 系列函数**：减少资源泄漏风险。
- **避免全局变量**：使用 `platform_set_drvdata()` / `platform_get_drvdata()` 保存私有数据。
- **支持 runtime PM**（如适用）：通过 `dev_pm_ops` 实现电源管理。

#### 3. 调试技巧

- 查看设备树是否正确加载：
  ```bash
  cat /proc/device-tree/...      # 原始设备树
  dtc -I fs -O dts /proc/device-tree > system.dts  # 反编译查看
  ```
- 查看 platform device 是否注册：
  ```bash
  ls /sys/bus/platform/devices/
  ```
- 查看驱动是否加载：
  ```bash
  dmesg | grep my-device
  ```

#### 4. 兼容性与可移植性

- 驱动应尽量通过设备树获取所有硬件信息，避免板级代码（board file）。
- 多个 compatible 字符串可用于兼容不同版本硬件：
  ```c
  { .compatible = "mycompany,my-device-v2", .data = &v2_data },
  { .compatible = "mycompany,my-device", .data = &v1_data },
  ```
  在 probe 中通过 `of_device_get_match_data()` 获取私有数据。





### 手动获取设备树资源

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/of.h> 														
#include <linux/of_gpio.h> 														
#include <linux/gpio.h>															
#include <dt-bindings/gpio/gpio.h>	

/*
	node3399 {
		compatible = "xyd_3399";
		reg = <0xFF76037c 0x4
			0xFF77e000 0x4
			0xFF780004 0x4
			0xFF780000 0x4
		>;
		testprop,mytest;
		test_list_string = "red fish","green fish", "blue fish";
		gpios = <&gpio1 4 GPIO_ACTIVE_HIGH>;
		interrupt-parent = <&gpio2>;
		interrupts = <7 IRQ_TYPE_EDGE_BOTH>; // 上升沿和下降沿触发
		key-irq-gpio = <&gpio2 7 IRQ_TYPE_EDGE_BOTH>; // GPIO2_A7  71
		xyd-led3 { //LED3
			gpios = <&gpio2 9 GPIO_ACTIVE_LOW>; //73
			label = "led3";
		};
	};

*/

static int __init hello_init(void) 
{
	int i;
	int ret;
    int count;
	//节点对象
	struct device_node *np = NULL, *child_np = NULL;
	//键对象
	struct property *prop = NULL;
	u32 regdata[8];
	int gpiono;
	enum of_gpio_flags flags;

	//1.通过绝对路径获得设备节点对象
	np = of_find_node_by_path("/xydnode3399");
	if (np) {
		printk("name=%s\n", np->name);
		printk("full name=%s\n", np->full_name);
	} else {
		printk("of_find_node_by_path fail\n");
		return -EFAULT;
	}

	//2.获取节点下的属性对象
	prop = of_find_property(np, "compatible", NULL);
	if (prop) {
		printk("name=%s\n", prop->name);
		printk("value=%s\n", (char *)prop->value);
	} else {
		printk("of_find_property fail\n");
		return -EFAULT;
	}
	//3.获取节点
	prop = of_find_property(np, "testprop,mytest", NULL);
	if (prop) {
		printk("name=%s\n", prop->name);
	} else {
		printk("of_find_property fail\n");
		return -EFAULT;
	}
    //4.获取字符串列表
	if((count = of_property_count_strings(np, "test_list_string")))
	{
		const char *test_list_string[count];
		for(i = 0; i < count; i++) {
			if (of_property_read_string_index(np, "test_list_string", i, &test_list_string[i]) == 0) {
				printk("test_list_string[%d]: %s\n", i, test_list_string[i]);
			}
		}
	}
	//5.获取reg键值；数组
	ret = of_property_read_u32_array(np, "reg", regdata, ARRAY_SIZE(regdata));
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(regdata); i++) {
			printk("regdata[%d]=0x%x\n", i , regdata[i]);
		}
	} else {
		printk("of_property_read_u32_array fail\n");
		return -EINVAL;
	}
	//6.获取gpio引脚号
	gpiono = of_get_named_gpio_flags(np, "gpios", 0, &flags);
	if (gpio_is_valid(gpiono)) {
		printk("gpiono=%d\n", gpiono);
		printk("flags==GPIO_ACTIVE_HIGH: %d\n", flags==GPIO_ACTIVE_HIGH);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		return -EINVAL;
	}
	//7.获取gpio引脚号
	gpiono = of_get_named_gpio_flags(np, "key-irq-gpio", 0, &flags);
	if (gpio_is_valid(gpiono)) {
		printk("gpiono=%d\n", gpiono);
		printk("flags==IRQ_TYPE_EDGE_BOTH: %d\n", flags==IRQ_TYPE_EDGE_BOTH);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		return -EINVAL;
	}

	//解析子节点
	
	//通过绝对路径获得设备节点对象
#if 0
	child_np = of_find_node_by_path("/node3399/xyd-led3");
#else
	child_np = of_get_child_by_name(np, "xyd-led3");
#endif


	if (child_np) {
		printk("name=%s\n", child_np->name);
		printk("full name=%s\n", child_np->full_name);
		gpiono = of_get_named_gpio_flags(child_np, "gpios", 0, &flags);
		if (gpio_is_valid(gpiono)) {
			printk("gpiono=%d\n", gpiono);
			
			printk("flags==GPIO_ACTIVE_LOW: %d\n", flags==GPIO_ACTIVE_LOW);
		} else {
			printk("of_get_named_gpio_flags fail\n");
			return -EINVAL;
		}
		
	} else {
		printk("of_find_node_by_path fail\n");
		return -EFAULT;
	}

	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;
}

static void __exit hello_exit(void) 
{
	printk("call %s() @ %d\n", __FUNCTION__, __LINE__);
}


//入口
module_init(hello_init);
//出口
module_exit(hello_exit);
MODULE_LICENSE("GPL"); 
```



- **获取GPIO引脚**

设备树节点

```dtd
myled_ctrl {	
    status = "okay";
    compatible = "rk3399_led2_green";
    gpio_led = <&gpio1 23 GPIO_ACTIVE_HIGH>; // GPIO1_C7 
};	
```

代码

```c
//获取匹配的device_node对象
struct device_node *led_ctrl_node = pdev->dev.of_node;
//拿到引脚
// 使用 of_get_named_gpio(button_ctrl_node, "gpios", 0);或下列函数
gpio_led = of_get_named_gpio_flags(led_ctrl_node, "gpio_led", 0, &flags);
if (!gpio_is_valid(gpio_led)) {
    printk("of_get_named_gpio_flags fail\n");
    return -EINVAL;
} 
printk("gpio_led=%d\n", gpio_led);
printk("flags==GPIO_ACTIVE_HIGH: %d\n", flags==GPIO_ACTIVE_HIGH);
```










### 现代设备树匹配方式（主流）

设备信息由设备树描述，内核在启动时自动解析并注册 `platform_device`。驱动通过 `of_match_table` 自动匹配。

```c
#include <linux/platform_device.h>
#include <linux/of.h>
#include <linux/of_address.h>
#include <linux/of_irq.h>

static int my_probe(struct platform_device *pdev)
{
    struct resource *res;
    void __iomem *base;
    int irq;

    // 获取寄存器资源
    res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
    if (!res)
        return -EINVAL;

    base = devm_ioremap_resource(&pdev->dev, res);
    if (IS_ERR(base))
        return PTR_ERR(base);

    // 获取中断
    irq = platform_get_irq(pdev, 0);
    if (irq < 0)
        return irq;

    // 其他初始化：时钟、复位、DMA 等
    // ...

    dev_info(&pdev->dev, "Device probed successfully\n");
    return 0;
}

static int my_remove(struct platform_device *pdev)
{
    // 执行清理（若未使用 devm_ 系列函数）
    return 0;
}

static const struct of_device_id my_of_match[] = {
    { .compatible = "vendor,my-device" },
    { }
};
MODULE_DEVICE_TABLE(of, my_of_match);

static struct platform_driver my_driver = {
    .probe = my_probe,
    .remove = my_remove,
    .driver = {
        .name = "my-device",
        .of_match_table = my_of_match,
        .owner = THIS_MODULE,
    },
};

module_platform_driver(my_driver); // 自动注册/注销
MODULE_LICENSE("GPL");
```



### 设备驱动匹配规则

`platform.c`框架源码

```c
static int platform_match(struct device *dev, struct device_driver *drv)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct platform_driver *pdrv = to_platform_driver(drv);

	/* When driver_override is set, only bind to the matching driver */
	if (pdev->driver_override)
		return !strcmp(pdev->driver_override, drv->name);

	/* Attempt an OF style match first */    //1.设备树of_match_table匹配
	if (of_driver_match_device(dev, drv))
		return 1;

	/* Then try ACPI style match */
	if (acpi_driver_match_device(dev, drv))
		return 1;

	/* Then try to match against the id table */     //2.平台总线驱动id_table列表匹配
	if (pdrv->id_table)
		return platform_match_id(pdrv->id_table, pdev) != NULL;

	/* fall-back to driver name match */            //3.平台总线驱动名匹配
	return (strcmp(pdev->name, drv->name) == 0);
}
```



### 获取中断配置

**方法1**

设备树主文件

```c
pinctrl: pinctrl {
    compatible = "rockchip,rk3399-pinctrl";
    rockchip,grf = <&grf>;
    rockchip,pmu = <&pmugrf>;
    #address-cells = <2>;
    #size-cells = <2>;
    ranges;

    gpio0: gpio0@ff720000 {
        compatible = "rockchip,gpio-bank";
        reg = <0x0 0xff720000 0x0 0x100>;
        clocks = <&pmucru PCLK_GPIO0_PMU>;
        interrupts = <GIC_SPI 14 IRQ_TYPE_LEVEL_HIGH 0>;

        gpio-controller;		//声明GPIO控制器
        #gpio-cells = <0x2>;

        interrupt-controller;      //声明中断控制器
        #interrupt-cells = <0x2>;
    };
};
```

自定义设备节点

```c
pwr_button {				
    compatible = "rk3399,power-button"; 			
    interrupt-parent = <&gpio0>;			//对应的上级中断控制器是 gpio0
    interrupts = <5 IRQ_TYPE_EDGE_BOTH>; 	//引脚号 触发方式 GPIO0_A5
};
```

代码获取中断资源

```c
static int irqno;   //中断号

//中断处理函数
static irqreturn_t button_handler(int irqno, void *dev)
{
	printk("call %s() @ %d\n", __func__, __LINE__);
	return IRQ_HANDLED;
}

//当平台设备和平台驱动匹配时会调用probe函数, 参数pdev就是匹配上平台设备对象
static int button_probe(struct platform_device *pdev)
{
	
	int ret;
	int trigger_type;
	//1.从platform_device的设备树节点中拿到中断号
	irqno = of_irq_get(pdev->dev.of_node, 0);
	if (irqno <= 0) {
		printk("of_irq_get fail\n");
		return -EFAULT;
	}
	printk("irqno=%d\n", irqno);
    
	//2.使用中断号拿到触发类型
	trigger_type = irq_get_trigger_type(irqno);
	if (trigger_type == IRQ_TYPE_EDGE_BOTH) {
		printk("双边沿触发\n");
	}
	
	//3.申请中断注册中断函数
	ret = request_irq(irqno, button_handler, trigger_type, "power_key_int", NULL);
	if (ret) {
		printk("request_irq fail\n");
		return -EFAULT;
	}

	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;
}

//当平台设备和平台驱动分离时会调用remove函数
static int button_remove(struct platform_device *pdev)
{
    //释放中断
	free_irq(irqno, NULL);
	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;

}
```

**方法2**

```c
//使用platform_get_resource函数拿到平台设备中的resource属性，从resource中取中断号
struct resource *irq_res = platform_get_resource(pdev, IORESOURCE_IRQ, 0);
irqno = irq_res->start;
```

**方法3**

```c
//直接从平台设备对象中拿
irqno = platform_get_irq(pdev, 0);
```







## 十三、PINCTL子系统



**核心定位与职责**

| 子系统      | 定位                   | 主要职责                                                     |
| ----------- | ---------------------- | ------------------------------------------------------------ |
| **pinctrl** | **引脚控制器子系统**   | 管理引脚的**复用功能（mux）** 和 **电气属性配置（config）**<br>例如：将物理引脚配置为 UART-TX、I2C-SCL 或 GPIO；设置上拉、驱动强度等 |
| **GPIO**    | **通用输入输出子系统** | 管理引脚作为 **GPIO（通用输入/输出）功能** 时的行为<br>例如：读取 GPIO 电平、设置输出高低、申请中断等 |

> ✅ **简单类比**：  
>
> - **pinctrl** 决定“这个引脚**是什么角色**”（是 UART 还是 GPIO）  
> - **GPIO** 决定“当它是 GPIO 时，**怎么操作它**”（读、写、中断）



- **pinctrl 是底层**：必须先将引脚**复用为 GPIO 功能**（如 `RK_FUNC_0`），GPIO 子系统才能操作它。
- **GPIO 依赖 pinctrl**：如果没有 pinctrl（或引脚未设为 GPIO），`gpio_request()` 会失败。



### pinctrl：描述“引脚该怎么配”

```c
pinctrl_uart1: uart1-pins {
    rockchip,pins = <
        2 11 RK_FUNC_1 &pcfg_pull_none   /* TX as UART1 */
        2 12 RK_FUNC_1 &pcfg_pull_up     /* RX as UART1 */
    >;
};

uart1: serial@ff190000 {
    pinctrl-names = "default";
    pinctrl-0 = <&pinctrl_uart1>;   // 只用 pinctrl，不涉及 GPIO
};
```

### 描述“作为 GPIO 怎么用”

```c
my_led {
    gpios = <&gpio2 9 GPIO_ACTIVE_LOW>;  // 使用 GPIO2_9
    // 注意：这个引脚必须在 pinctrl 中配置为 GPIO 功能！
};
&pinctrl {			
    key1_button {
        button_k1_gpio:button_k1_gpio {
            rockchip,pins = <1 1 RK_FUNC_GPIO &pcfg_pull_up>;  //gpio1_A1 引脚功能配置为GPIO 内部上拉 
        };
    };
};
```

⚠️ **关键**：`gpios = <&gpio2 9 ...>` 中的 `gpio2` 是一个 **GPIO controller 节点**，而该节点内部通常会引用 pinctrl：

> ```c
> gpio2: gpio2@ff720000 {
>     compatible = "rockchip,rk3399-gpio";
>     gpio-controller;			// 声明为gpio控制器
>     #gpio-cells = <2>;
>     gpio-ranges = <&pinctrl 0 0 32>;  // 告诉 GPIO 子系统：这32个GPIO由pinctrl管理
> };
> ```



**案例1**：配置引脚的工作方式

```dtd
&pinctrl {
	key1_button {
		button_k1_gpio:button_k1_gpio {  //定义三个按钮为上拉输出
			rockchip,pins = <1 1 RK_FUNC_GPIO &pcfg_pull_up>,
							<1 3 RK_FUNC_GPIO &pcfg_pull_up>,
							<1 4 RK_FUNC_GPIO &pcfg_pull_up>;
		};
	};
};
```

**案例2：**同时使用三个引脚的中断

```dtd
key1_button {					
    compatible = "rk3399,key1-button";
    gpios = <&gpio1 1 IRQ_TYPE_EDGE_BOTH>; //GPIO1_A1
    pinctrl-names = "default";
    pinctrl-0 = <&button_k1_gpio>;
};
//add k1								
key1_button {								
    compatible = "rk3399,key1-button";								
    gpios = <&gpio1 1 IRQ_TYPE_EDGE_BOTH>; //GPIO1_A1								
    gpios3 = <&gpio1 3 IRQ_TYPE_EDGE_BOTH>; //GPIO1_A3								
    gpios4 = <&gpio1 4 IRQ_TYPE_EDGE_BOTH>; //GPIO1_A4								
    pinctrl-names = "default";								
    pinctrl-0 = <&button_k1_gpio>;								
};
```

代码

```c
static int button_probe(struct platform_device *pdev)
{
	enum of_gpio_flags  flags;
	int ret;
	//1. 获取匹配的设备树节点对象
	struct device_node *button_ctrl_node = pdev->dev.of_node;
	gpio_button = of_get_named_gpio_flags(button_ctrl_node, "gpios", 0, &flags);
	if (gpio_is_valid(gpio_button)) {
		printk("gpio_button=%d\n", gpio_button);
		printk("flags==GPIO_ACTIVE_LOW:%d\n", flags==GPIO_ACTIVE_LOW);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		return -EFAULT;
	}
	//获取中断号
	irqno1 = gpio_to_irq(gpio_button);
	printk("k1 irqno = %d\n", irqno1);
	//申请中断
	ret = request_irq(irqno1, button_handler, IRQ_TYPE_EDGE_BOTH, "k1_key_xyd", "k1");
	if (ret) {
		printk("request_irq fail\n");
		return -EFAULT;
	}

	gpio_button = of_get_named_gpio_flags(button_ctrl_node, "gpios3", 0, &flags);
	if (gpio_is_valid(gpio_button)) {
		printk("gpio_button=%d\n", gpio_button);
		printk("flags==GPIO_ACTIVE_LOW:%d\n", flags==GPIO_ACTIVE_LOW);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		return -EFAULT;
	}
	//获取中断号
	irqno2 = gpio_to_irq(gpio_button);
	printk("k2 irqno = %d\n", irqno2);
	//申请中断
	ret = request_irq(irqno2, button_handler, IRQ_TYPE_EDGE_BOTH, "k2_key_xyd", "k2");
	if (ret) {
		printk("request_irq fail\n");
		return -EFAULT;
	}

	gpio_button = of_get_named_gpio_flags(button_ctrl_node, "gpios4", 0, &flags);
	if (gpio_is_valid(gpio_button)) {
		printk("gpio_button=%d\n", gpio_button);
		printk("flags==GPIO_ACTIVE_LOW:%d\n", flags==GPIO_ACTIVE_LOW);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		return -EFAULT;
	}
	//获取中断号
	irqno3 = gpio_to_irq(gpio_button);
	printk("k3 irqno = %d\n", irqno3);
	//申请中断
	ret = request_irq(irqno3, button_handler, IRQ_TYPE_EDGE_BOTH, "k3_key_xyd", "k3");
	if (ret) {
		printk("request_irq fail\n");
		return -EFAULT;
	}

	printk("<kernel> call %s()\n", __func__);
	return 0;
}
static int button_remove(struct platform_device *pdev)
{
	free_irq(irqno3,"k3");
	free_irq(irqno2,"k2");
	free_irq(irqno1,"k1");
	printk("<kernel> call %s()\n", __func__);
	return 0;
}
```





## 十四、属性文件操作



**设备属性文件**

---

通过属性文件驱动可以生成一个额外的普通文件进行驱动文件的读写，方便没有系统调用时操作驱动（read，write）

直接使用`cat`或`echo`可进行驱动控制操作

实现属性文件的核心机制是 **`struct device_attribute`** 和 **`struct driver_attribute`**，分别对应 **设备属性** 和 **驱动属性**。

**核心结构体**

```c
struct device_attribute {
	struct attribute	attr;
	ssize_t (*show)(struct device *dev, struct device_attribute *attr,
			char *buf);
	ssize_t (*store)(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count);
};
```



### 案例实现

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/platform_device.h>
#include <linux/of.h> 												
#include <linux/of_gpio.h> 												
#include <linux/gpio.h>															
#include <dt-bindings/gpio/gpio.h>	


static int gpio_led;
static int gpio_value;

ssize_t gpio_led_show(struct device *dev, struct device_attribute *attr, 	char *buf)
{
	printk("call %s() @ %d\n", __func__, __LINE__);
	return sprintf(buf, "led %d level\n", gpio_get_value(gpio_led));
}

ssize_t gpio_led_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret, value;
	ret = kstrtoint(buf, 10, &value); //字符串转10进制整数
	if (ret) {
		printk("kstrtoint fail\n");
		return -EFAULT;
	}

	printk("call %s() @ %d  value=%d\n", __func__, __LINE__, value);

	if (value == 1) {
		gpio_direction_output(gpio_led, gpio_value);
		printk("led on\n");
	} else if(value == 0) {
		gpio_direction_output(gpio_led, !gpio_value);
		printk("led off\n");
	} else {
		printk("i only support 0 or 1 ctrl led\n");
	}

	return count; //不能返回return 
}


//1.创建设备属性对象并初始化
//创建struct device_attribute对象dev_attr_gpio_led
static DEVICE_ATTR(gpio_led, 0664, gpio_led_show, gpio_led_store);


//平台总线probe函数
static int led_probe(struct platform_device *pdev)
{
	enum of_gpio_flags flags;
	
	struct device_node *led_ctrl_node = pdev->dev.of_node;
	gpio_led = of_get_named_gpio_flags(led_ctrl_node, "gpio_led", 0, &flags);
	if (gpio_is_valid(gpio_led)) {
		printk("gpio_led=%d\n", gpio_led);
		printk("flags==GPIO_ACTIVE_HIGH: %d\n", flags==GPIO_ACTIVE_HIGH);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		return -EINVAL;
	}

	gpio_value = (flags==GPIO_ACTIVE_HIGH );
	gpio_request(gpio_led, "led2_green");
	gpio_direction_output(gpio_led, !gpio_value);
	gpio_free(gpio_led);

	//2.创建gpio_led属性文件	类似创建设备文件
	device_create_file(&pdev->dev, &dev_attr_gpio_led);
	
	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;
}

static int led_remove(struct platform_device *pdev)
{
	//销毁gpio_led属性文件
	device_remove_file(&pdev->dev, &dev_attr_gpio_led);
	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;
}

static const struct of_device_id led_ctrl_match_table[] = {
	{ .compatible = "rk3399_led2_green", }, //和设备节点compatible属性匹配
	{}
};

struct platform_driver led_driver = {
	.probe	= led_probe,
	.remove	= led_remove,
	.driver	= {
		.name	= "rk_led", //平台驱动名称,又是匹配条件
		.of_match_table = led_ctrl_match_table,
	},
};

static int __init drv_mod_init(void) 
{
	printk("call %s() @ %d\n", __func__, __LINE__);
	return platform_driver_register(&led_driver);
}

static void __exit drv_mod_exit(void) 
{
	printk("call %s() @ %d\n", __FUNCTION__, __LINE__);
	platform_driver_unregister(&led_driver);
}

module_init(drv_mod_init);
module_exit(drv_mod_exit);
MODULE_LICENSE("GPL"); 

```



### 关键函数

#### DEVICE_ATTR

创建一个拼接的`struct device_attribute`对象

```c
//device.h
//参数1：名称拼接
//参数2：文件权限码
//参数3：show回调函数
//参数4：store回调函数
static DEVICE_ATTR(gpio_led, 0664, gpio_led_show, gpio_led_store);
```

宏展开

```c
#define DEVICE_ATTR(_name, _mode, _show, _store) \
	struct device_attribute dev_attr_##_name = __ATTR(_name, _mode, _show, _store)
```

```c
static struct device_attribute dev_attr_gpio_led = {
    .attr = {.name = "gpio_led", .mode = 0644},
    .show = gpio_led_show,
    .store = gpio_led_store,
};
```

**注册/注销**属性在 `probe()` 和 `remove()` 中



#### device_create_file

创建属性文件

```c
device_create_file(&pdev->dev, &dev_attr_gpio_led);
```

#### device_remove_file

销毁属性文件

```c
device_remove_file(&pdev->dev, &dev_attr_gpio_led);
```



### 用户空间访问

```bash
# 写入
echo 42 > /sys/devices/platform/my-device/my_value
# 读取
cat /sys/devices/platform/my-device/my_value
```





**驱动属性文件**

---



适用于**驱动全局配置**（不绑定具体设备）。

1. **定义驱动属性**

```c
static ssize_t debug_level_show(struct device_driver *drv, char *buf)
{
    return sprintf(buf, "%d\n", global_debug_level);
}

static ssize_t debug_level_store(struct device_driver *drv,
                                 const char *buf, size_t count)
{
    return kstrtoint(buf, 10, &global_debug_level) ? -EINVAL : count;
}

static DRIVER_ATTR_RW(debug_level);
```



2. **注册到 platform_driver**

```c
static struct platform_driver my_driver = {
    .probe = my_probe,
    .remove = my_remove,
    .driver = {
        .name = "my-device",
        .of_match_table = my_of_match,
    },
};

// 在模块初始化时创建
static int __init my_init(void)
{
    int ret = platform_driver_register(&my_driver);
    if (ret)
        return ret;

    // 创建驱动级属性
    ret = driver_create_file(&my_driver.driver, &driver_attr_debug_level);
    if (ret)
        platform_driver_unregister(&my_driver);
    return ret;
}

static void __exit my_exit(void)
{
    driver_remove_file(&my_driver.driver, &driver_attr_debug_level);
    platform_driver_unregister(&my_driver);
}
```



3. **用户空间访问**

```c
cat /sys/bus/platform/drivers/my-device/debug_level
echo 2 > /sys/bus/platform/drivers/my-device/debug_level
```



### 高级用法：多个属性 & 自动创建

```c
static struct attribute *my_attrs[] = {
    &dev_attr_my_value.attr,
    &dev_attr_status.attr,
    NULL,
};

static const struct attribute_group my_attr_group = {
    .attrs = my_attrs,
};

// 在 probe 中：
sysfs_create_group(&pdev->dev.kobj, &my_attr_group);

// 在 remove 中：
sysfs_remove_group(&pdev->dev.kobj, &my_attr_group);
```





## 十五、输入子系统

Linux 输入子系统（Input Subsystem）是内核中用于**统一管理各类输入设备**（如按键、鼠标、触摸屏、游戏手柄、加速度计等）的核心框架。它将硬件差异抽象化，为用户空间提供统一的事件接口（`/dev/input/eventX`），并支持内核空间事件上报。

### 三大组件

1. **设备驱动层**（Driver）  
   - 负责与硬件交互（读取 GPIO、I2C、SPI 等）
   - 调用 Input Core API 上报事件
2. **输入核心层**（Input Core）  
   - 位于 `drivers/input/`，提供通用接口（`input_allocate_device`, `input_event` 等）
   - 管理设备注册、事件分发、设备号分配
3. **事件处理层**（Event Handler）  
   - 如 `evdev`（生成 `/dev/input/eventX`）
   - 如 `mousedev`（兼容老式鼠标）、`joydev`（游戏手柄）

> ✅ **用户空间通过 `/dev/input/eventX` 以统一格式读取事件**，无需关心底层是 GPIO 按键还是 USB 鼠标。



**核心数据结构**

`struct input_dev`

```c
struct input_dev {
    const char *name;           // 设备名（如 "gpio-keys"）
    const char *phys;           // 物理路径（如 "gpio/0"）
    unsigned long evbit[BITS_TO_LONGS(EV_CNT)];     // 支持的事件类型
    unsigned long keybit[BITS_TO_LONGS(KEY_CNT)];   // 支持的按键
    unsigned long relbit[BITS_TO_LONGS(REL_CNT)];   // 相对坐标（鼠标）
    unsigned long absbit[BITS_TO_LONGS(ABS_CNT)];   // 绝对坐标（触摸屏）
    // ... 其他如 swbit（开关）、mscbit（杂项）
};
```





### 事件类型

| 宏       | 说明     | 常见用法           |
| -------- | -------- | ------------------ |
| `EV_KEY` | 按键事件 | 按键、开关         |
| `EV_REL` | 相对坐标 | 鼠标移动（dx, dy） |
| `EV_ABS` | 绝对坐标 | 触摸屏、摇杆       |
| `EV_SW`  | 开关状态 | 翻盖、电源按钮     |
| `EV_MSC` | 杂项事件 | 扫描码等           |



### 按键/坐标码（`keybit`, `absbit`）

- `KEY_POWER`, `KEY_VOLUMEUP`（定义在 `include/uapi/linux/input-event-codes.h`）
- `ABS_X`, `ABS_Y`, `ABS_MT_POSITION_X`（多点触控）



### 常用函数

#### 分配与释放设备

```c
struct input_dev *input_allocate_device(void);
void input_free_device(struct input_dev *dev);
```

> ✅ 必须成对使用（类似 `kzalloc` / `kfree`）。




#### 注册/注销设备

```c
int input_register_device(struct input_dev *dev);
void input_unregister_device(struct input_dev *dev);
```

- `input_register_device()` 会：
  - 分配次设备号（`event0`, `event1`...）
  - 创建 sysfs 节点（`/sys/class/input/inputX/`）
  - 通知事件处理器（如 `evdev`）创建 `/dev/input/eventX`

> ✅ **推荐使用 `devm_input_allocate_device()` + 自动注销**。



### 设置设备能力（Capabilities）

```c
// 设置支持的事件类型
__set_bit(EV_KEY, input_dev->evbit);
__set_bit(EV_ABS, input_dev->evbit);

// 设置支持的按键
__set_bit(KEY_POWER, input_dev->keybit);
__set_bit(KEY_HOME, input_dev->keybit);

// 设置绝对坐标范围（如触摸屏）
input_set_abs_params(input_dev, ABS_X, 0, 1023, 0, 0);
input_set_abs_params(input_dev, ABS_Y, 0, 767, 0, 0);
// 参数：dev, axis, min, max, fuzz, flat
```

> ⚠️ **不要直接操作 `keybit` 数组**！使用 `__set_bit()` 宏（来自 `asm/bitops.h`）。





### 上报输入事件

#### 通用事件上报

```c
void input_event(struct input_dev *dev, unsigned int type,
                 unsigned int code, int value);
```

- `type`: `EV_KEY`, `EV_REL`, `EV_ABS`...
- `code`: `KEY_POWER`, `ABS_X`...
- `value`:
  - `EV_KEY`: 0=release, 1=press, 2=repeat
  - `EV_ABS`: 坐标值（如 512）
  - `EV_REL`: 相对变化量（如 +10）



#### 专用上报函数（推荐）

```c
// 按键
input_report_key(dev, KEY_POWER, 1);      // 按下
input_report_key(dev, KEY_POWER, 0);      // 释放

// 绝对坐标
input_report_abs(dev, ABS_X, 512);
input_report_abs(dev, ABS_Y, 384);

// 相对坐标
input_report_rel(dev, REL_X, 10);
input_report_rel(dev, REL_Y, -5);

// 同步事件（必须调用！）
input_sync(dev);
```

> ✅ **必须以 `input_sync()` 结束一组事件**，表示“当前事件包已完整”，用户空间才会收到。



#### 多点触控（MT）上报（Linux 3.0+）

```c
// 开始一个触点
input_mt_slot(dev, 0);                     // slot 0
input_mt_report_slot_state(dev, MT_TOOL_FINGER, true); // 触点存在
input_report_abs(dev, ABS_MT_POSITION_X, 100);
input_report_abs(dev, ABS_MT_POSITION_Y, 200);

// 第二个触点
input_mt_slot(dev, 1);
input_mt_report_slot_state(dev, MT_TOOL_FINGER, true);
input_report_abs(dev, ABS_MT_POSITION_X, 300);
input_report_abs(dev, ABS_MT_POSITION_Y, 400);

input_sync(dev);
```

> 📌 需先设置 MT 能力：
>
> ```c
> input_set_capability(dev, EV_ABS, ABS_MT_POSITION_X);
> input_set_capability(dev, EV_ABS, ABS_MT_POSITION_Y);
> input_mt_init_slots(dev, 2, 0); // 支持2个触点
> ```



### 资源管理（推荐 `devm_` 版本）

为避免资源泄漏，使用 managed resource API：

```c
static int my_probe(struct platform_device *pdev)
{
    struct input_dev *input;
    int error;

    // 自动释放 input_dev
    input = devm_input_allocate_device(&pdev->dev);
    if (!input)
        return -ENOMEM;

    input->name = "my-touchscreen";
    input->dev.parent = &pdev->dev;

    __set_bit(EV_ABS, input->evbit);
    input_set_abs_params(input, ABS_X, 0, 4095, 0, 0);
    // ... 其他设置

    error = devm_input_register_device(input);
    if (error)
        return error;

    platform_set_drvdata(pdev, input);
    return 0;
}
// 退出时自动调用 input_unregister_device + input_free_device
```

> ✅ **强烈推荐使用 `devm_input_allocate_device()` 和 `devm_input_register_device()`**。



### 用户空间接口

#### 1. 设备节点

- `/dev/input/event0`, `/dev/input/event1`, ...
- 权限通常为 `crw-rw---- root input`

#### 2. 事件格式（struct input_event）

```c
struct input_event {
    struct timeval time;  // 时间戳
    __u16 type;           // EV_KEY, EV_ABS...
    __u16 code;           // KEY_POWER, ABS_X...
    __s32 value;          // 状态值
};
```

#### 3.测试工具

```bash
# 查看设备信息
cat /proc/bus/input/devices
# 实时监听事件
sudo evtest /dev/input/event0
# 或使用 hexdump
sudo od -tx1 -An /dev/input/event0
```



### 实践案例

```dtd
pwr_button {				
    compatible = "rk3399,power-button"; 			
    interrupt-parent = <&gpio0>;//对应的上级中断控制器是 gpio0													
    interrupts = <5 IRQ_TYPE_EDGE_BOTH>; //中断号 触发方式 GPIO0 A5	
};
```

```c
#include <linux/module.h>
#include <linux/init.h>
#include <linux/platform_device.h>
#include <linux/of.h>												
#include <linux/of_gpio.h>											
#include <linux/gpio.h>															
#include <dt-bindings/gpio/gpio.h>	
#include <linux/interrupt.h>
#include <linux/input.h>

//1.声明一个输入设备对象的指针
static struct input_dev *button_dev;
static int gpio_button;
static int irqno;

static irqreturn_t button_handler(int irqno, void *dev)
{
	int value;
	value = gpio_get_value(gpio_button);

	if (value) {
		//5.上报事件对象
		input_report_key(button_dev, KEY_A, 0); //上报事件 键值松开0
	} else {
		input_report_key(button_dev, KEY_A, 1); //上报事件 键值按下1
	}
	input_sync(button_dev); //同步事件,表示上报完毕
	
	
	printk("call %s() @ %d\n", __func__, __LINE__);
	return IRQ_HANDLED;
}


//当平台设备和平台驱动匹配时会调用probe函数, 参数pdev就是匹配上平台设备对象
static int button_probe(struct platform_device *pdev)
{
	
	int ret;
	
	//获取匹配的device_node对象
	struct device_node *button_ctrl_node = pdev->dev.of_node;
	
	//2.创建输入设备对象
	button_dev = input_allocate_device();
	if (!button_dev) {
		printk("input_allocate_device fail: Not enough memory\n");
		return -ENOMEM;
	}
	//3.初始化输入设备对象
	//set_bit(EV_KEY, button_dev->evbit); //为输入设备对象设置事件类型 EV_KEY 按键事件
	button_dev->evbit[0] = BIT_MASK(EV_KEY);

	//set_bit(KEY_A, button_dev->keybit); //为输入设备对象设置键码 KEY_A
	button_dev->keybit[BIT_WORD(KEY_A)] = BIT_MASK(KEY_A);																										

	button_dev->name = "xyd power key button";
	button_dev->phys = "rk3399 key";
	button_dev->id.vendor = 0x11;
	button_dev->id.product = 0x22;
	button_dev->id.version = 51;
	
	//4.注册输入设备对象
	ret = input_register_device(button_dev);
	if (ret) {
		printk("input_register_device: Failed to register device\n");
		ret = -EBUSY;
		goto err_input_register_device;
	}

	gpio_button = of_get_named_gpio(button_ctrl_node, "gpios", 0);
	if (gpio_is_valid(gpio_button)) {
		printk("gpio_button=%d\n", gpio_button);
	} else {
		printk("of_get_named_gpio_flags fail\n");
		ret= -EINVAL;
		goto err_input_unregister;
	}

	//获取中断号
	irqno = gpio_to_irq(gpio_button);
	//申请中断
	if (request_irq(irqno, button_handler, IRQ_TYPE_EDGE_BOTH, "power-key int", NULL)) {
			printk("request irq Can't allocate irq %d\n", irqno);
			ret = -EBUSY;
			goto err_input_unregister;
	}
	
	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;

err_input_unregister:
	input_unregister_device(button_dev);

err_input_register_device:
	input_free_device(button_dev);
	return ret;
}

//当平台设备和平台驱动分离时会调用remove函数
static int button_remove(struct platform_device *pdev)
{
	free_irq(irqno, NULL);
	input_unregister_device(button_dev);
	input_free_device(button_dev);

	printk("call %s() @ %d\n", __func__, __LINE__);
	return 0;

}

static const struct of_device_id button_ctrl_match_table[] = {
	{ .compatible = "rk3399,power-button", }, //和设备节点compatible属性匹配
	{}
};

struct platform_driver button_driver = {
	.probe	= button_probe,
	.remove	= button_remove,
	.driver	= {
		.name	= "rk_button", //平台驱动名称,又是匹配条件
		.of_match_table = button_ctrl_match_table,
	},
};

static int __init drv_mod_init(void) 
{
	printk("call %s() @ %d\n", __func__, __LINE__);
	return platform_driver_register(&button_driver);
}

static void __exit drv_mod_exit(void) 
{
	printk("call %s() @ %d\n", __FUNCTION__, __LINE__);
	platform_driver_unregister(&button_driver);
}

module_init(drv_mod_init);
module_exit(drv_mod_exit);
MODULE_LICENSE("GPL"); 

```





### 扩展：输入设备属性（sysfs）

可通过 sysfs 查看设备能力：

```bash
# 查看支持的事件类型
cat /sys/class/input/input0/capabilities/ev
cat /sys/class/input/input0/capabilities/key
```



### 注意事项

#### ✅ 正确做法

1. **必须调用 `input_sync()`**：否则事件不会上报。
2. **使用 `devm_` API**：避免资源泄漏。
3. **设置合理的 `fuzz/flat`**：用于滤除噪声（如触摸屏抖动）。
4. **多点触控用 Slot 协议**：避免旧式 `ABS_MT_*` 报点混乱。



## 十六、将驱动移植进内核



### 操作步骤

1. **将测试好的驱动代码放到内核目录中**

```bash
lj@ubuntu:~/work/rk3399-android-8.1/kernel/drivers/xyd$ ll
total 12
drwxr-xr-x   2 robin robin 4096 Dec 18 19:39 ./
drwxrwxr-x 137 robin robin 4096 Dec 18 19:37 ../
-rwxr--r--   1 robin robin    0 Dec 18 19:38 button_input_drv.c*
-rwxr--r--   1 robin robin    0 Dec 18 19:39 Kconfig*
-rwxr--r--   1 robin robin   97 Dec 18 19:40 Makefile*
```

2. **编辑Makefile**

```makefile
#
# Makefile for xyd input button.
#

obj-$(CONFIG_XYD_KEY1_BUTTON)	+= button_input_drv.o


```

3. **拷贝一个Kconfig并修改为当前驱动选项**

```makefile
#
# GPIO subsystem configuration
#

menu "GPIO KEY1 support"

config XYD_KEY1_BUTTON
	tristate "gpio_key1 support"
	---help---
	  gpio key1 support。

	  dvicestree ->
	  	pwr_button {				
			compatible = "rk3399,power-button"; 			
			gpios = <&gpio0 5 GPIO_ACTIVE_LOW>; 			
		};	


endmenu

```

4. **在该驱动上级目录中进行包含**

`vim ../Makefile`

```makefile
obj-y				+= xyd/
```

`vim ../Kconfig`

```makefile
source "drivers/xyd/Kconfig"
```

5. **重新配置内核菜单**

根据目录结构选中需要固化的驱动程序

6. **编译内核**

执行编译命令

7. **刷入内核后进行设备驱动验证**

验证系统目录中是否存在设备和驱动文件



### 配置细节说明

- Kconfig
  - `tristate`：支持 `Y`（built-in）、`M`（module）、`N`（disable）
    - 若配置为 `Y`：驱动代码会链接进 `vmlinux`
    - 若配置为 `M`：生成 `drivers/char/my_led/my_led.ko`
  - `depends on`：声明依赖（如 `GPIOLIB`, `OF`）
  - `help`：帮助文本（会在 menuconfig 中显示）
- Makefile
  - `CONFIG_MY_LED` 是 Kconfig 中定义的选项
  - 当配置为 `Y` 时，`obj-y += my_led.o` → 编译进内核
  - 当配置为 `M` 时，`obj-m += my_led.o` → 编译为模块
  - 末尾的 `/` 表示这是一个子目录。



## 十七、安卓系统开发



### 安卓刷机

1.查看编译脚本

```bash
robin@ubuntu:~/work/rk3399-android-8.1$ ll build-nanopc-t4.sh
lrwxrwxrwx 1 robin robin 41 May 15  2021 build-nanopc-t4.sh -> device/rockchip/rk3399/build-nanopc-t4.sh*
```

2.运行脚本

```bash
robin@ubuntu:~/work/rk3399-android-8.1$ ./build-nanopc-t4.sh -h
Usage: ./build-nanopc-t4.sh [ARGS]

Options:
  -a         build Android
  -B         build U-Boot
  -K         build Linux kernel
  -W         build Wi-Fi drivers (.ko)

  -F, --all  build all (U-Boot, kernel, wifi, Android)
  -M         make rockdev image
  -u         generate update.img

  -h         show this help message and exit

```

3.完全编译

```bash
robin@ubuntu:~/work/rk3399-android-8.1$ ./build-nanopc-t4.sh -F -M
```

4.使用官方工具刷入固件

```bash
robin@ubuntu:~/work/rk3399-android-8.1$ ll rockdev/
total 12
drwxrwxr-x  3 robin robin 4096 Dec 18 19:15 ./
drwxr-xr-x 40 robin robin 4096 Oct 26 23:49 ../
drwxrwxr-x  2 robin robin 4096 Dec 18 19:15 Image-nanopc_t4/      #固件目录
```



### 编译安卓用户态C程序



1.**创建一个`Android.mk`文件并添加如下内容**

```makefile
# Copyright 2006 The Android Open Source Project

#获取当前路径
LOCAL_PATH:= $(call my-dir)
#清空以LOCAL_开头的变量，LOCAL_PATH除外
include $(CLEAR_VARS)
#指定源文件
LOCAL_SRC_FILES:= \
	button_input_test.c
#在任何情况都会编译这个模块
LOCAL_MODULE_TAGS := optional
#指定目标文件
LOCAL_MODULE:= button_input_test_elf
#编译成可执行程序
include $(BUILD_EXECUTABLE)

```

2.将用户态源码复制到Android目录下

3.配置编译环境

```bash
robin@ubuntu:~/work/rk3399-android-8.1$ source build/envsetup.sh    #加载变量到环境
including device/rockchip/rk3399/vendorsetup.sh
including sdk/bash_completion/adb.bash

robin@ubuntu:~/work/rk3399-android-8.1$lunch             #选择运行环境
```

4.开始编译

```bash
robin@ubuntu:~/work/rk3399-android-8.1$ mmm .button/
[100% 488/488] Install: out/target/product/rk3399/system/bin/button_input_test_elf
```

5.将编译出的程序传到开发板中

```bash
adb root
adb disable-verity
adb reboot
adb root
adb remount
adb push ./out/target/product/rk3399/system/bin/button_input_test_elf /system
```

赋予执行权限即可在Android中运行程序



