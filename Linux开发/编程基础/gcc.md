...
# 同时创建导入库（可选）
dlltool --dllname mathutils.dll --def mathutils.def --output-lib libmathutils.a
```
...
> 这里相当于定义一个空的宏(占位符)，一般用于预处理代码识别展开
...
**`-f`**选项是一个功能强大的“开关”家族，主要用于控制编译过程中的各种具体行为，例如启用或禁用特定的语言特性、优化技术、代码生成规则以及诊断检查等

**开启某项功能**：使用 `-f<feature>`，例如 `-fpic`。

**关闭某项功能**：使用 `-fno-<feature>`，例如 `-fno-pic`。很多选项都有对应的开启和关闭形式。
